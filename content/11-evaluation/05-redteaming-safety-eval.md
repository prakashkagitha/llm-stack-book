...
!!! interview "Interview Corner"
    ...
    **A:** ToxiGen and similar benchmarks measure the model's *default behavior* — what the model outputs when asked with no adversarial pressure. They capture harm that occurs in ordinary use. Dangerous-capability evaluations measure what the model *could* do rather than what it does by default. WMDP is the cheap static end of that spectrum — a multiple-choice probe for latent hazardous knowledge in bio, cyber, and chem, scored by log-likelihood with no generation and no attack — while full capability elicitation (expert-scored CBRN uplift studies, agentic evals like AgentHarm or InterCode-CTF) applies the strongest available jailbreaks, scaffolding, and tool access to find the model's ceiling. They ask: if a determined bad actor tries their hardest, what can this model help them do? You need both because a model can have low ToxiGen scores (it doesn't produce hate speech by default) while still having high biosecurity uplift potential under targeted attack. Conversely, a model can be brittle to jailbreaks on low-stakes content (high ASR on HarmBench harassment behaviors under GCG or PAIR) while genuinely lacking the domain knowledge to provide CBRN uplift. Together, the two evaluation types give you different risk profiles: ordinary-use risk and tail-risk from adversarial actors.
