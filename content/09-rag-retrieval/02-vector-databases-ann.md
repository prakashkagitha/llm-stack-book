...
- **FAISS** (Facebook AI Similarity Search) — the canonical *library*, not a database. It implements every index above (Flat, IVF, PQ, HNSW, and combinations) with CPU and GPU backends. If you build retrieval infra, you will use or reimplement FAISS. It has no server, no durability or transactional CRUD, and no metadata store — you can serialize an index to a file (`write_index`) and restrict a search to an id set (`IDSelector*` via `SearchParameters`), but crash recovery, replication, and mapping structured predicates onto ids are yours to build.
