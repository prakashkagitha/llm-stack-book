...
??? note "Solution"
    ...
    $$
    p_c = \frac{4000}{10000} = 0.40, \qquad p_t = \frac{4200}{10000} = 0.42
    $$

    Pooled proportion (used for the null-hypothesis standard error):

    $$
    p_{\text{pool}} = \frac{4000 + 4200}{10000 + 10000} = \frac{8200}{20000} = 0.41
    $$

    Pooled standard error:

    $$
    \text{SE} = \sqrt{p_{\text{pool}}(1 - p_{\text{pool}})\left(\tfrac{1}{n_c} + \tfrac{1}{n_t}\right)}
    = \sqrt{0.41 \times 0.59 \times \left(\tfrac{1}{10000} + \tfrac{1}{10000}\right)}
    $$

    $$
    = \sqrt{0.2419 \times 0.0002} = \sqrt{0.00004838} \approx 0.006956
    $$

    Test statistic:

    $$
    z = \frac{p_t - p_c}{\text{SE}} = \frac{0.42 - 0.40}{0.006956} = \frac{0.02}{0.006956} \approx 2.88
    $$
...
??? note "Solution"
    **(a) Plain A/B.** Using the chapter's formula:
    ...
    $$
    n = \frac{(z_{\alpha/2} + z_\beta)^2 \cdot 2\sigma^2}{\delta^2}
    = \frac{(1.96 + 0.84)^2 \cdot 2 \times 0.49^2}{0.02^2}
    $$

    $$
    = \frac{7.84 \times 0.4802}{0.0004} = \frac{3.765}{0.0004} \approx 9{,}413 \text{ users per arm.}
    $$
    ...
    **(b) With CUPED.** CUPED multiplies the metric variance by $(1 - \rho^2)$. With $\rho = 0.7$:
    ...
    $$
    n_{\text{CUPED}} = 9{,}413 \times 0.51 \approx 4{,}801 \text{ users per arm.}
    $$

    At 500 users/arm/day:

    $$
    \frac{4{,}801}{500} \approx 9.6 \text{ days},
    $$
...
??? note "Solution"
    **(a)** Under the null, each test independently avoids a false positive with probability $1 - 0.05 = 0.95$. Across 10 independent tests:
    ...
    $$
    P(\text{at least one false positive}) = 1 - 0.95^{10} = 1 - 0.5987 \approx 0.40.
    $$
    ...
    **(b)** Bonferroni divides the target family-wise rate by the number of tests:
    ...
    $$
    \alpha_{\text{per-test}} = \frac{0.05}{10} = 0.005.
    $$

    Requiring $p < 0.005$ per test gives a family-wise error rate of

    $$
    1 - (1 - 0.005)^{10} = 1 - 0.995^{10} = 1 - 0.9511 \approx 0.049 < 0.05,
    $$
