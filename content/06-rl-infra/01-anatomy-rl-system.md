...
| Arithmetic intensity | very low (one weight read per generated token ⇒ ≈$B$ FLOPs/byte in bf16, $B$ = decode batch) | high (≳$10^3$ FLOPs/byte: grows with tokens per micro-batch) |
...
| Precision | often weight-only quantized (INT8/FP8) | bf16 compute weights + fp32 master weights & optim state |
...
In the synchronous loop, stages run *in series*: generate, then score, then learn. While the **inference** engine generates, the **training** engine's GPUs (with their fat optimizer state) sit idle. While the **learner** trains, the inference engine sits idle. If generation is 70% of the step and training 20%, then under a strict disaggregated split your training GPUs are idle ~80% of the time (everything that is not the 20% training phase) and your inference GPUs idle ~30% of the time. On a cluster costing thousands of USD/hour, that idle time is the headline cost of RL.
