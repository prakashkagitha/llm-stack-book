...
The cleanest fix: make the generation batch *larger than the number of concurrent slots* and let continuous batching pull from the queue. If an engine holds 64 concurrent sequences but you give it 512 prompts to produce, freed slots from short sequences immediately admit new prompts, and the engine stays full until the queue drains. Now the only bubble is the *final* tail — the last few long sequences with no replacements. With a deep queue, $\eta_{\text{gen}}$ climbs from $\bar L/L_{\max}$ toward

$$
\eta_{\text{gen}} \;\approx\; \frac{\sum_i L_i}{\sum_i L_i \;+\; \text{slots}\cdot L_{\max}}
$$

where the second term in the denominator is the worst-case idle slot-time while the final long sequence drains alone. Only when the queue is deep relative to the tail ($\text{slots}\cdot L_{\max} \ll \sum_i L_i$) does this reduce to the first-order form $1 - (\text{slots}\cdot L_{\max})/\sum_i L_i$ — outside that regime the first-order version is meaningless (it goes negative), while the expression above always lies in $(0,1]$.
...
!!! example "Worked example: filling the bubble on an 8×H100 GRPO step"
    ...
    **+ Partial rollout, 4k budget/phase (tail control).** The 14k outliers no longer gate a step; they amortize across ~4 phases. Note what this does and does not buy: it does *not* reduce the tokens you must generate, it removes the final serial drain, so the phase falls back to its token-bound floor and stops depending on the length outliers at all. Per-phase generation ≈ **115 s** ($\approx 4.5\text{M}/40{,}000$), step ≈ 115 s, and step time becomes *predictable* instead of a random variable set by the longest trace.

    **+ Dynamic sampling (DAPO).** Statistically, suppose 35% of groups are all-correct/all-wrong and contribute zero gradient. Without filtering you *trained* on 2,048 sequences but only $0.65 \times 2048 \approx 1{,}331$ carried signal — your *effective* useful throughput was 65% of nominal. With dynamic sampling you oversample ~1.54× to refill the batch with useful groups. Because generation is already async+oversubscribed, that extra sampling never *serializes* against training — but it is real token work, and generation is now token-bound, so the step stretches to ≈ **175 s** while delivering **256 fully-useful groups (2,048 sequences)** instead of ~166 useful groups' worth of signal.

    Net: the three scheduling levers take the step from **260 s → ~115 s (≈2.3× wall-clock)** with no change to what you train on; dynamic sampling then spends ~1.5× more generation (step ≈ 175 s) so that *every* trained group moves the policy — trading tokens for gradient quality, which DAPO reports pays for itself in steps-to-target. Among the scheduling levers the single biggest win was overlap (Lever 3, −60 s, vs −50 s for oversubscription and −35 s for partial rollout); the single biggest *quality* win was dynamic sampling.
...
!!! sota "State of the Art & Resources (2026)"
    ...
    **Foundational work**
    ...
    - [DeepSeek-AI, *DeepSeek-R1: Incentivizing Reasoning Capability in LLMs via Reinforcement Learning* (2025)](https://arxiv.org/abs/2501.12948) — scaled GRPO (introduced earlier in [*DeepSeekMath*, 2024](https://arxiv.org/abs/2402.03300)) to large-scale RLVR, the baseline that all tricks in this chapter build upon.
    ...
    **Recent advances (2023–2026)**
    ...
    - [Noukhovitch et al., *Asynchronous RLHF: Faster and More Efficient Off-Policy RL for Language Models* (ICLR 2025)](https://arxiv.org/abs/2410.18252) — formal treatment of overlapping generation and training; ~40–70% wall-clock speedup with empirically negligible quality cost at staleness 1 (an experimental result across model scales and tasks, not a bound).
