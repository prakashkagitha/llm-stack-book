...
class SearchQAEnv:
    ...
    Terminal reward = 1.0 for a correct answer, plus a per-turn step
    penalty to discourage dithering.
    """
    SYSTEM_PROMPT = (
    ...
    def step(self, action_text: str) -> tuple[str, float, bool]:
        ...
        if srch:
            ...
            # Out of turns mid-search = failure: no task reward, only the
            # step penalties already accumulated (here -0.02 per turn).
