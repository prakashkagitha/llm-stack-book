...
GPT-2's 2019 pattern (Radford et al.) includes ` ?\p{N}+`: an unbounded run of digits, merged greedily like any other substring. The consequence is idiosyncratic number tokens — a single token for `2020` because it was frequent in the crawl, two (`'20'` + `'31'`) for `2031` because it wasn't, and three (`'123'` `'45'` `'67'`) for `1234567`. Arithmetic then becomes a task whose input segmentation differs between numerically adjacent inputs, which is a genuinely bad property. Every frontier tokenizer since GPT-4's `cl100k_base` — including Llama 3, Qwen, and Gemma — caps digit runs for exactly this reason. `capstone/PLAN.md` §8 makes **integer arithmetic** the RLVR task and §7 does math capability injection, so this is not theoretical: it directly gates two later chapters.
...
!!! note "Aside: `\p{L}`/`\p{N}` need the `regex` package — and the repo makes that optional"
    ...
    The two differ only on characters in Unicode categories `Nl`/`No` (Roman numerals, `½`), which `\p{N}` treats as numbers and `\d` does not. Under the stdlib port they are matched by the *letter* branch instead — `\w` covers anything `str.isalnum()` accepts, so `[^\W\d_]` = letters + `Nl` + `No` — which means they are glued to adjacent letters and, unlike the three-digit-capped `\p{N}{1,3}`, run unbounded: `'a½b'` is one chunk `['a½b']` under the stdlib pattern versus `['a', '½', 'b']` under the `regex` one. Both are total (verified below). **A real run should install `regex`** — the pattern the artifact is trained under is written into the artifact, and the loader warns if the module's default differs, so the two flavors can never be silently mixed.
...
Three observations. First, the cache is nearly free and worth ~4.4×: on a 400 KB slice, 98,693 pre-token chunks reduced to 9,472 distinct ones — a **90.4% hit rate**, and it climbs as the cache warms over a real shard. Second, tokenization is embarrassingly parallel at document granularity, so plain `multiprocessing` gets the pure-Python path into "one coffee break" territory and — on this corpus — beats HF's Rust `encode_batch`. We did not chase that result down to certainty — one plausible contributor is caching policy: our Python workers keep an *unbounded* per-process chunk cache, while `tokenizers`' Rust BPE keeps a bounded per-word cache sized by the `cache_capacity` constructor argument. Do not read the row as "Python beats Rust"; read it as "at 8 MB, process-level parallelism and a warm cache dominate." Third, the compiled encoders are still the right answer for production, and you can have them *without abandoning the tokenizer you just trained*, because the merge table is portable (next section).
...
[Chapter 14.11](../14-capstone/11-evaluation-and-serving.html) quantizes Stack-100M to int4 and runs it on a laptop with `llama.cpp`. Its `convert_hf_to_gguf.py` does not read your pre-tokenizer regex out of `tokenizer.json`. Instead it tokenizes a fixed probe string, hashes the result, and looks the hash up in a hardcoded table of known BPE pre-tokenizers (`llama-bpe`, `gpt-2`, `deepseek-llm`, …). A vocabulary nobody has seen before never matches, so `get_vocab_base_pre()` prints a loud "BPE pre-tokenizer was not recognized" block (including the exact hash it computed) and then **aborts** with `NotImplementedError` — no GGUF is produced at all. This is the one export gotcha that is *not* silent, and that is a mercy: the corrupting version lives on the C++ side, where a GGUF whose `tokenizer.ggml.pre` key is missing is loaded with the **default GPT-2 pre-tokenizer** — a genuinely different pattern from ours (unbounded digit runs, no case-insensitive contractions). All you get there is one load-time `missing pre-tokenizer type, using: 'default'` warning in a wall of loader output; the model then runs, tokenizes differently from the one that was trained, and the laptop build produces quiet nonsense. Get there by patching the converter to skip the hash check, or by hand-rolling the GGUF.
...
!!! key "Key Takeaways"
    ...
    - Three export gotchas, two of them silent: `bytes_to_unicode` must be byte-exact; HF extracts `AddedToken`s regardless of `add_special_tokens=False` (the real lever is `split_special_tokens=True` / `encode_special_tokens = True`); and llama.cpp identifies BPE pre-tokenizers by a *hash* against a hardcoded table — an unknown vocabulary makes `convert_hf_to_gguf.py` abort with `NotImplementedError` until you register your hash, while a GGUF missing its `tokenizer.ggml.pre` key falls back on the C++ side to GPT-2's regex behind a single load-time warning.
    ...
    - At 100M parameters vocabulary size is a real design lever: GPT-2's 50,257 would cost roughly a quarter of the budget — ≈9 layers' worth of parameters, 4 layers more than 32,768 costs — to buy ~1.2% more compression, and tied embeddings (Press & Wolf, 2017) are worth ~6 layers of depth for one line of code.
