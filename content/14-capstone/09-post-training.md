...
Conversations vary wildly in length; padding each to `max_seq_len=2048` would waste most of the batch on `<|pad|>`. As in pretraining we **pack**: concatenate rendered conversations end-to-end and slice into fixed-length windows. The subtlety is that packing must carry the mask *and* prevent cross-conversation attention — a token in conversation B must not attend to conversation A sharing its window. We solve this exactly as in [Chat Templates, Data Formatting & Sequence Packing](../05-posttraining-alignment/02-chat-templates-packing.html) and Chapter 14.2: a per-token `seq_id` that resets the document-aware attention mask at each conversation boundary, so no token can read across it. One honest difference from the pretraining packer of Ch. 14.2: there we also shipped per-document `position_ids`; here we pass only `seq_ids`, so RoPE positions run 0…`block-1` straight across the window. That is safe precisely *because* the mask already blocks cross-conversation attention — RoPE scores depend only on position *differences*, so a constant per-conversation offset cancels inside every attention logit that survives the mask. (Pass `position_ids` as well if you want byte-for-byte parity with the pretraining path.)
...
class PackedSFTDataset(Dataset):
    ...
    blocks cross-conversation attention. Positions are NOT reset per
    conversation (no position_ids are passed); with RoPE that is free, since
    the surviving attention scores depend only on position differences.
    """
...
def sft_train(model, loader, *, epochs=3, lr=2e-5, warmup=100,
    ...
    lr=2e-5 is ~150x below the pretraining AdamW peak (3e-3) and ~1000x below
    the Muon peak (2e-2): we are nudging a converged model. Too large an LR here erases pretrained knowledge (the
...
!!! warning "Common pitfall: the off-by-one mask that silently trains on the prompt"
    ...
    The same invariant is worth checking in *any* framework you use. TRL's `SFTTrainer` implements assistant-only masking with `assistant_only_loss=True`, which requires the tokenizer's chat template to wrap assistant content in a `{% generation %}` block so that `apply_chat_template(..., return_assistant_tokens_mask=True)` can return the mask. If the template lacks that block, `apply_chat_template` warns and returns an **all-zero** `assistant_masks`, so *every* label becomes `-100` and the run trains on **nothing** — a degenerate loss and a checkpoint identical to the input, not the prompt leakage you might expect. (Recent TRL releases catch this for you: they swap in a `{% generation %}`-annotated training template, or raise when an example has no assistant tokens.) Print one decoded batch and check.
...
where $\rho_{i,t} = \dfrac{\pi_\theta(o_{i,t}\mid q, o_{i,<t})}{\pi_{\theta_{\text{old}}}(o_{i,t}\mid q, o_{i,<t})}$ is the per-token importance ratio between the current policy and the policy that generated the rollouts. When $\hat A_i>0$ (this sample beat its group) the objective pushes $\rho$ up (make these tokens more likely); when $\hat A_i<0$ it pushes them down; the clip prevents any single update from moving too far. **The structural fact that governs everything downstream: a group in which all rewards are equal has zero std, hence zero advantage on every token, hence contributes exactly zero *policy* gradient.** (With $\beta_{\text{KL}}>0$ the group is not quite *free*: the surrogate term vanishes, but the KL penalty is still a function of $\pi_\theta$, so a degenerate group contributes a pure pull back toward the reference with no reward signal opposing it — one more reason to drop such groups outright.) That is a feature when the group is uninformative and a trap when it is the norm — which is the cold-start problem, quantified in Exercise 5 and fixed by dynamic sampling below.
...
Two reporting consequences follow, neither a footnote. First, the loop's `degen {n_degenerate}/{prompts_per_iter}` denominator must become `tries` — the number of prompts *drawn* — since `prompts_per_iter` is now the number of groups *kept*. Read as a fraction, that log line is the *discard rate* $(\text{tries}-\text{kept})/\text{tries}$, which is not the table's `tries / len(groups)` but its reciprocal complement: $\text{tries}/\text{kept} = 1/(1 - \text{discard rate})$ — a discard rate of $0.67$ is an oversample of $3.0$, not of $0.67$. Print `tries / max(1, len(groups))` directly if you want to read the table's band off the log. Second, `n_correct / n_total` is now conditioned on a *filtered* population that excludes both the all-wrong and the all-right prompts, so it is biased toward 50% and will look flat while the model genuinely improves. Evaluate on a fixed held-out prompt set (Ch. 14.11) and treat the training curve purely as a health signal.
...
??? note "Solution"
    ...
    (c) With $R = [1,1,1,1,1]$: $\bar R = 1$, std $= 0$. Every advantage is $(1 - 1)/(0 + \varepsilon) = 0$, so the surrogate is zero and this prompt contributes **no policy gradient** (with `kl_beta > 0` all that survives is the KL pull toward the reference) — the all-correct degenerate case, mirror image of all-wrong. DAPO's dynamic sampling exists precisely so you never pay for a rollout like this one.
...
??? note "Solution"
    ...
    Two readings. First, dynamic sampling is *cheap exactly where you do not need it and expensive exactly where you do* — but the expensive case is the one where the alternative (plain GRPO) is taking two-thirds of its gradient steps on zeros, so you are paying generation to buy real gradient rather than paying it for nothing. Second, this is precisely why `collect_nondegenerate_groups` caps the budget at `oversample=3.0`: at $p=0.05$ the expected cost of $47.5$ prompts sits right at the $16\times3=48$ cap. An *expectation* at the cap is not "it just fits" — that negative-binomial count has standard deviation $\sqrt{N(1-q)}/q = \sqrt{16\times0.663}/0.337 \approx 9.7$, so the budget binds roughly *half* the iterations and the function returns a **short** batch, silently shrinking that iteration's effective batch size with no error raised. So check `len(kept) < target_groups`, not just the `tries/len(kept)` ratio; either one screams that the task is out of reach. If you hit the cap, the answer is not a bigger budget; it is more SFT, easier prompts, or a curriculum (see [RL Data, Curriculum & Replay Management](../06-rl-infra/12-rl-data-curriculum-replay.html)).
