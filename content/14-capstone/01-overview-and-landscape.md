...
| Logit/loss head, unchunked | ≈30 GB | $B{\cdot}s \times V$ held four times over — bf16 logits + fp32 upcast + fp32 `log_softmax` output + a second fp32 copy for the z-loss, ≈14 B/element (Ch. 14.7 itemizes it) — Ch. 14.4's chunked loss cuts it to ~1 GB |
...
!!! example "Worked example: FLOPs to GPU-hours to dollars"
    ...
    **Total training compute for the stable phase:**
    ...
    versus $1.095 \times 10^{19}$ from the naive $6ND$ — the true cost is 31% *higher* than the napkin figure (equivalently, $6ND$ under-counts the real total by 24%), and you would have paid that difference in wall-clock.
    ...
    **Convert to wall-clock.** The A100's bf16 Tensor Core dense peak is $\pi = 312$ TFLOP/s $= 3.12\times10^{14}$ FLOP/s. Define MFU (Model FLOPs Utilization) as achieved model FLOP/s divided by $\pi$, where "model FLOPs" is exactly the attention-inclusive $C$ above. Chapter 14.7's instrumented run sustains ≈228k tokens/s on a `torch.compile`d loop with an SDPA/FlashAttention backend, which is **MFU ≈ 0.58** in this convention (0.45 under 6ND-only). A less-tuned loop — no compile, a smaller micro-batch, an unfused attention path — sits near **0.45** (0.34 under 6ND). So:
    ...
    $$
    t_{\text{tuned}} = \frac{1.434\times10^{19}}{0.58 \times 3.12\times10^{14}} \approx 7.9\times10^{4}\ \text{s} \approx 21.9\ \text{hours}
    $$
...
At `d_model=512`, the largest GEMM in a block is the SwiGLU up/gate projection: $(B\cdot s, 512) \times (512, 1408)$. The $K$ dimension of 512 is small by tensor-core standards, so each matmul spends a larger fraction of its time on prologue/epilogue and memory traffic than the same FLOPs would in a $d_{\text{model}}=4096$ model. Then multiply the *count* of kernels by 30 layers: 30 RMSNorm pairs, 30 RoPE applications, 30 SiLU-and-multiply gates, 30 residual adds, all of them memory-bandwidth-bound and all of them paying kernel-launch latency. Doubling the width to 1024 quadruples the per-layer cost ($\propto d^2$) *and* doubles the tied embedding, so an iso-parameter 1024-wide model has only about 6 layers and would do its arithmetic in roughly a *fifth* as many launches. **Deep-and-thin buys loss-per-parameter and pays for it in launch count** — which is fine, because you train once and serve forever, but it is why a tuned run lands near 0.58 rather than the 0.7+ a wide model can reach, and why an untuned one collapses to 0.45 or below.
...
!!! key "Key Takeaways"
    ...
    - The flagship pretraining pass is **≈22–29 GPU-hours** (≈25 planning) at MFU 0.45–0.58 attention-inclusive, equivalently 0.34–0.45 under $6ND$ — **≈USD 25–50**. The whole project lands near **USD 90–100**; Chapter 14.12 owns the itemized bill. Always quote MFU *with* its FLOP convention: 44.5% and 58.2% can describe the same run.
...
??? note "Solution"
    ...
    ```python
    # Delete the `if cfg.mtp_heads: raise` guard at the top of count_params(), then
    # put this where the existing `total = ...` line is -- i.e. AFTER `per_block`,
    # `all_blocks`, `final_norm` and `lm_head` have been computed.
    #
