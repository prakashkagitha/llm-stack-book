...
Because we *tie* the embedding to the output projection (Press & Wolf, *Using the Output Embedding to Improve Language Models*, 2017), this matrix is counted **once**, not twice. Untied, we would pay another 16.78M — about a sixth of the model — for a table that empirically performs no better at this scale.
...
!!! example "Worked example: the honest FLOP budget, and the KV cache"
    ...
    **Training FLOPs.** The famous rule is $6ND$ FLOPs for $N$ non-embedding parameters over $D$ tokens (see [Scaling Laws: Kaplan, Chinchilla & Beyond](../03-pretraining/04-scaling-laws.html)). But $6ND$ counts only the *body's parameter matmuls*. Two terms it omits are individually negligible at 7B and jointly enormous at our shape:
    ...
    PLAN §0's canonical **≈22–29 GPU-hr** stable-phase band is the well-utilized end of this table — it is quoted for the **18B-token stable phase** ($C = 1.434\times10^{19}$, i.e. 12.8 h at 100% MFU), so it needs $\approx$45–58% attention-inclusive model-FLOPs utilization ($12.8/29$ to $12.8/22$; equivalently 0.34–0.45 under the bare-$6ND$ convention Ch. 14.7 also reports, which there counts $N$ as the *total* 101.4M parameters, not $N_{\text{body}}$), which at 100M requires `torch.compile`, fused cross-entropy and a FlashAttention kernel all switched on (Ch. 14.7 measures what you actually get, and lands at 58.2% attention-inclusive — 21.9 h over the 18B stable phase, 24.4 h over the full 20B budget, both at the fast end of the band). At \$1–2/GPU-hr the dollar envelope is **\$25–\$50**. This formula is also the *numerator* of Ch. 14.7's MFU meter (MFU = FLOP/token × tokens/s ÷ peak FLOP/s; the peak is the denominator) — use the three-term version there, or your MFU will read 36% low, because bare $6N_{\text{body}}$ is only 64% of the true count.
...
!!! sota "State of the Art & Resources (2026)"
    ...
    **Stability and the QK-norm lineage**
    ...
    - OLMo 2 (AI2, 2024), Qwen3 (Alibaba, 2025), Gemma 3 (Google DeepMind, 2025) and Chameleon (Meta, 2024) — open models that ship QK-norm before RoPE (Qwen3, Gemma 3 and Chameleon per head; OLMo 2 over the full concatenated head axis); Qwen3 is the export target here.
...
??? note "Solution"
    ...
    (c) A100 peak $\times$ 3600 s $= 1.123\times10^{18}$ FLOP/hour, so hours $= 14.2/\text{MFU}$. At 40% that is **35.5 GPU-hours** (not the 22.6 that $6ND$ alone predicts). PLAN §0's 22–29 GPU-hour band is quoted for the **18B-token stable phase** ($C = 1.434\times10^{19}$, i.e. 12.8 h at 100% MFU), so landing inside it needs MFU between $12.8/29 \approx 44\%$ and $12.8/22 \approx 58\%$ — the ≈45–58% band of the worked example; the measured loop's 58.2% attention-inclusive utilization gives 21.9 h over the stable phase (24.4 h over the full 20B budget), at the fast end. At \$1–2/GPU-hr the \$25–\$50 envelope holds, and Ch. 14.7 measures which end you land on.
