...
1. **Settle the serving surface**: which inference path every measurement runs through, what Ch. 14.4's `KVCache` and `logits_to_keep` already give you, and the one string-level wrapper this chapter adds on top.
2. **Evaluate** the final checkpoint against a small, fixed battery: held-out perplexity, an arithmetic probe, a tiny multiple-choice set, retrieval-QA exact match, and — the capstone's flagship deliverable — the ReAct **agent loop** itself. Report numbers, not vibes.
3. **Write down what the model cannot do.** This is not optional padding — it is the deliverable that makes the numbers trustworthy.
4. **Quantize** the checkpoint to int8, then int4, understanding exactly what post-training quantization (PTQ) does to the weights and why round-to-nearest (RTN) is a baseline rather than the state of the art.
5. **Re-run the battery on the quantized model** and tabulate the deltas. The claim "int8 is free, int4 costs you something on the hard tail" is an empirical claim; this chapter makes you check it rather than believe it.
6. **Export and serve** on CPU, with measured (not estimated) latency and memory on your own machine, and a named upgrade path into the real 2026 quantization/serving stack (torchao, llm-compressor / compressed-tensors, GGUF + llama.cpp, vLLM).
...
!!! warning "Common pitfall: `logits_to_keep=1` is a sampling optimization, not a default"
    ...
    The flip side is memory: an all-positions logits tensor is $B \times T \times V \times 4$ bytes in fp32. At $B=4$, $T=2047$, $V=32768$ that is **1.07 GB** — on a CPU eval box the logits, not the weights, are your memory ceiling. Keep the eval batch size small (1–4 on CPU), or use `cfg.loss_chunk > 0` and Ch. 14.4's chunked fused loss when you only need the loss.
...
from stacklm.tokenizer.bpe import SPECIAL_TOKENS        # Ch. 14.3
...
def generate_fn(model, tokenizer, prompt, max_new_tokens=8, temperature=0.0, **kw):
...
!!! tip "Practitioner tip: the KV cache is not a rounding error at 100M"
    ...
    `KVCache(cfg, batch_size=1, max_seq=2048, device="cpu", dtype=torch.bfloat16).nbytes()` returns **31,457,280** bytes — 30 MiB, or ≈31.5 MB. Hold that next to the ≈63 MB int4 weight budget we will fight for in Section 7: at the pretrain context the cache is *half the model*, and after mid-training extends the context to 8192 (Ch. 14.8) it is $15{,}360 \times 8192 \approx 126$ MB — **twice the model**. Small models are relatively *more* KV-bound than large ones, because the cache scales with layers × context while the weights scale with layers × width².

    Two consequences. (1) Keep the cache in **bf16**, which is Ch. 14.4's default and costs nothing measurable: keys and values are activations, re-read once per step and never accumulated. fp32 doubles it for no benefit. (2) Everything past that is a serving-systems problem — fp8/int8 KV quantization, paged blocks, prefix reuse across requests — and lives in [PagedAttention & KV-Cache Memory Management](../04-kernels-efficiency/06-paged-attention-kv.html) and [Prefix Caching & KV-Cache Reuse](../07-inference-serving/07-prefix-caching.html).

!!! warning "Re-run the cache-equivalence test after quantization"

    Ch. 14.4's CI proves that *the fp32 model's* cached decode matches its full recompute. That proof does not transfer to the quantized model, because Section 7 replaces every `nn.Linear` with a different module, and a packing or row-gather bug in `QuantizedLinear` shows up as fluent-but-wrong text rather than a crash. Re-run the same assertion against the quantized model at toy scale — it is three lines, and it is the difference between "the export loads" and "the export is correct":
...
It is the single most trustworthy number in this chapter precisely because it is *cheap to make honest*: as long as your held-out shard was excluded from the training manifest (see the manifest/hash bookkeeping in [14.2 The Data Pipeline](02-data-pipeline.html)), there is no way to game it by prompt-crafting or answer-formatting tricks. It directly measures the thing pretraining optimizes. Its weakness is exactly the flip side: it tells you how well the model predicts *held-out web text*, not whether it can multiply two-digit numbers or drive a tool loop — for that you need Section 4.
...
def compute_perplexity(model: Stack100M, val_shard_dir: str, pad_id: int,
    ...
    Three details that decide whether the number is honest (or fits in RAM):
      ...
      3. DOCUMENT MASKING. Ch. 14.2 stores `position_ids` that reset to 0 at
         ...
         cross-document attention. Evaluating WITHOUT it would let the model
         peek across document boundaries it never saw in training and report a
         perplexity that does not match how the model is served.
    """
    ...
    for i, batch in enumerate(loader):
        ...
        with _autocast(device):
            logits, _ = model(input_ids, position_ids=position_ids,
                              seq_ids=seq_ids)              # (B, T, V) — logits_to_keep=0
...
Note what is *not* passed: `logits_to_keep`. Leaving it at its default of 0 is what makes `logits` a `(B, T, V)` tensor rather than the `(B, 1, V)` a sampling call would return — §2's pitfall, in the one place it does the most damage. Note also that `position_ids` and `seq_ids` both come straight from Ch. 14.2's packer and both matter: `position_ids` drives RoPE (so a document that starts mid-window is rotated from its own position 0, exactly as in training), and `seq_ids` builds the no-cross-document mask. Evaluate without them and you measure a model in a geometry it was never trained in.

!!! example "Worked example: reading a perplexity number"

    Suppose `compute_perplexity` reports a held-out loss of **2.9 nats/token**, in the "on the order of ~2.8–3.2 nats/token" range documented for the flagship run in [14.7](07-pretraining-run.html). That converts to $\text{PPL} = e^{2.9} \approx 18.2$. In plain terms: on held-out text from the training mix, the model's predictive distribution has an effective branching factor of about 18 tokens at each position — not "18 equally likely tokens" (real text is far more skewed), but a useful single-number summary of average surprise. A larger, better-trained model on the same mix might sit at PPL ≈ 8–12; a random-guessing model over the 32768-token vocabulary would sit at PPL ≈ 32768, i.e. a loss of $\ln 32768 \approx 10.4$ nats — which is exactly where the training curve *started* (Ch. 14.7). The honest framing: this number tells you the model learned real structure in FineWeb-Edu-like text, and nothing more.
...
!!! warning "Common pitfall: perplexity is not comparable across tokenizers"
    ...
    A perplexity number is defined per *token*, and Stack-100M's tokens are its own 32768-entry byte-level BPE (Ch. 14.3). Comparing PPL 18 here against PPL 12 from a model with a 128k vocabulary is meaningless — the second model's tokens are longer, so it makes fewer, harder predictions per unit of text. The tokenizer-independent version is **bits per byte**: $\text{BPB} = \frac{\text{total nll in nats}}{\ln(2)\cdot\text{total UTF-8 bytes}}$. If you ever want to compare Stack-100M against an external model, compute BPB on the same raw text, not PPL. This is the same trap the Pile/`lm-evaluation-harness` `bits_per_byte` metric exists to avoid.
...
They are chosen to probe *different* skills, and the asymmetry between them — together with §3's predictive fit — is the finding: reliable narrow reasoning that RLVR targeted (§4.1), broad parametric knowledge the model has no capacity for (§4.2), reading comprehension when the answer is handed over (§4.3), and multi-step tool use, the capstone's flagship deliverable (§4.4).
...
def make_arithmetic_probe(n: int = 200, seed: int = 0, max_digits: int = 2) -> list[dict]:
    ...
    Seeded, so the probe set is reproducible from the seed alone — a hashed,
    versioned eval set in the cheapest possible form."""
...
def eval_arithmetic(model, tokenizer, generate_fn, problems: list[dict]) -> dict:
    ...
    score exact match. Greedy (temperature=0) so the number is reproducible."""
    n_correct = 0
    for p in problems:
...
def sequence_logprob(model, tokenizer, prompt: str, continuation: str,
    ...
    The practical mitigation is the one baked into TINY_MC_SET below: put the
    separating space at the START of every continuation (" Paris", not "Paris"
    after a trailing space), so the join lands on a whitespace boundary that
    byte-level BPE pre-tokenization does not merge across.
    """
...
def eval_mc_probe(model, tokenizer, mc_set: list[dict] = TINY_MC_SET,
    ...
    """Reports BOTH metrics `lm-evaluation-harness` reports:
      acc      — raw summed log-prob (what the harness calls `acc`)
      acc_norm — summed log-prob divided by the continuation's UTF-8 BYTE length
                 (the harness's `acc_norm`), which removes the length bias that
                 otherwise favours whichever option happens to tokenize shortest.
    Report both. If they disagree, your choices are length-imbalanced and the
    raw number is measuring string length as much as knowledge."""
    n_raw = n_norm = 0
    for item in mc_set:
        raw, norm = [], []
...
def eval_retrieval_qa(model, tokenizer, generate_fn, retriever,
    ...
    would feed the model a Python repr and score ~0 for reasons that have
    nothing to do with the model.

    We also report RETRIEVER RECALL@1 separately. With k=1 this probe is a joint
    test of two components; without the recall number a bad EM is unattributable
    — you cannot tell whether the model failed to read or the retriever failed
    to fetch."""
    ...
    for pair in qa_pairs:
        ...
        # Did the retriever even put the answer in front of the model?
        n_retrieved += int(normalize_answer(pair["gold_answer"])
                           in normalize_answer(passage_text))
        prompt = (f"<|system|>Answer using only the passage below.<|end|>\n"
                  f"<|user|>Passage: {passage_text}\n"
                  f"Question: {pair['question']}<|end|>\n<|assistant|>")
...
Note the prompt reuses the exact chat frame the model was SFT'd on in [14.9](09-post-training.html) — `<|system|>` / `<|user|>` / `<|assistant|>`, no improvisation. §2's `generate_fn` enables the special tokens by default so those markers become single reserved ids rather than literal angle-bracket bytes; an eval that quietly reformats the chat template is measuring a distribution shift you introduced yourself, not the model.
...
def eval_agent(model, tok, env: ToolEnv, tasks: list[AgentTask],
    ...
    for task in tasks:
        ...
        turns.append(len(assistant))

        # Non-termination: burned every step without ever emitting a final answer.
        non_term = len(assistant) >= max_steps and (not acts or acts[-1].kind != "final")
        n_capped += int(non_term)
...
!!! tip "Practitioner tip: the libraries that own this layer"
    ...
    Three open-source harnesses do this properly, and you should graduate to them once your probe set outgrows a Python list:

    - **`lm-evaluation-harness`** (EleutherAI) — the de-facto standard for log-likelihood/cloze tasks; backs the Hugging Face Open LLM Leaderboard. You plug in a custom model by subclassing `lm_eval.api.model.LM` and implementing exactly two primitives — `loglikelihood(requests)` and `generate_until(requests)` — which are *precisely* `sequence_logprob` and `generate` from this chapter. That is not a coincidence; those two primitives span nearly all static LLM evaluation.
    ...
    - **`inspect_ai`** (UK AI Safety Institute) — the framework built for *agentic* evaluation: solvers, tool sandboxes, scorers, and full trace logging with a viewer. It formalizes exactly the decomposition in `eval_agent` above (per-step tool records, turn limits, scorer-per-metric). If your agent eval grows past one file, port it to `inspect_ai` rather than reinventing the trace format. Cross-link: [Reasoning, Coding & Agentic Evals](../11-evaluation/04-reasoning-coding-agentic-evals.html) and [Building Eval Harnesses](../11-evaluation/03-eval-harnesses.html).
...
!!! example "Worked example: a plausible probe report"
    ...
    | Retrieval-QA | exact match / recall@1 | on the order of 50-75% / >90% | ~50 |
    ...
    Notice the pattern such a table typically reveals. Arithmetic and retrieval-QA — narrowly scoped, RL/SFT-targeted, unambiguously gradable — score well above chance. The closed-book MC set, which requires broad parametric world knowledge the model never had capacity to store, sits closer to a coin flip. And the agent's *format* validity is far higher than its *end-to-end* success: distillation reliably teaches a 100M model the wire grammar, and much less reliably teaches it the policy. That gap — high format validity, mediocre task success — is the single most characteristic signature of a small distilled agent, and it should be reported, not smoothed over.
...
- **What it is not good at**: long-horizon reasoning, closed-book trivia outside the training mix's coverage, code beyond the toy StarCoder-subset flavor it saw, and — like every language model — it will hallucinate fluently with no internal signal of uncertainty a downstream system can cheaply detect.
- **The agent is format-reliable and policy-fragile.** Quote the §4.4 decomposition explicitly: a model that emits well-formed tool calls 95% of the time and solves 40% of tasks is *not* "40% as good as a real agent" — it is a model that has memorized a grammar and is guessing at a policy. Say that in the model card, because a downstream integrator who sees only the EM number will draw the wrong conclusion about where to add guardrails.
...
!!! warning "Contamination: the probe you build is the probe you must audit"
    ...
    Because you constructed the tiny MC set and the arithmetic generator yourself, contamination risk is lower than for a model evaluated on a public leaderboard — but it is not zero. Three concrete leaks to check for:
    ...
    2. **Retrieval-QA leakage.** If the small local corpus used for the retrieval probe overlaps with pretraining data verbatim, the model may already have the passage memorized and the probe silently degrades into a closed-book test wearing an open-book costume. Build the retrieval corpus from sources *excluded* from the pretraining manifest, the same discipline used for the held-out perplexity shard in §3.
    3. **Agent-task leakage into distillation.** The nastiest one, because it is self-inflicted: if the ~30 evaluation tasks overlap the tasks you generated teacher trajectories for in Ch. 14.10, you are measuring memorization of specific trajectories, not tool-use ability. Hold out the eval tasks *before* the distillation rollout, by hash, and never let them into the SFT mix.

    More generally: any number you cannot regenerate from a documented, hashed, versioned eval set is not a number — it is an anecdote. See [Statistical Rigor in Evaluation](../11-evaluation/06-statistical-rigor-eval.html) for confidence intervals on small-n probes. At $n = 30$ and an observed 40% success rate, a Wilson 95% interval spans roughly 25–58% — so a rival configuration scoring 50% is *not* meaningfully better, and reporting it as an improvement would be the single most common error in small-scale eval.

{{fig:honest-eval-four-probes-asymmetry}}

## 6. Post-Training Quantization: RTN, GPTQ, and AWQ

With honest numbers in hand, the second half of the chapter is serving. The fp32 checkpoint is ~405MB (101.3M quantizable parameters × 4 bytes — exact accounting in [14.4](04-architecture.html)); even at bf16 that is ~203MB, comfortably in RAM on any laptop but wasteful given how little precision a well-trained weight actually needs at inference time. **Post-training quantization (PTQ)** converts trained fp32/bf16 weights to low-bit integers *after* training, no gradient updates required — the natural complement to the quantization-aware and mixed-precision *training* techniques in [Mixed Precision, bf16 & FP8 Training](../03-pretraining/08-mixed-precision-fp8.html). This chapter is a hands-on companion to the book's dedicated PTQ chapters — [Quantization I: Post-Training Quantization (GPTQ, AWQ, SmoothQuant)](../04-kernels-efficiency/07-quantization-ptq.html) and [Quantization II: INT4/INT8/FP8, GGUF, bitsandbytes & QAT](../04-kernels-efficiency/08-quantization-formats-qat.html) — read those for full derivations; here we implement the baseline end to end and *use* it.
...
The update spreads each column's quantization error onto the columns not yet quantized, weighted by how strongly the Hessian says those columns interact — an efficient, layer-local instance of the classic Optimal Brain Surgeon idea (LeCun et al., 1990; Hassibi & Stork, 1993) applied to quantization instead of pruning. The payoff is that GPTQ can push to 4 or even 3 bits with far less quality loss than RTN, at the cost of calibration data and $O(d_{in}^3)$ work per layer for the Hessian inverse (GPTQ's real implementation batches this via Cholesky decomposition; the sketch below is the pedagogical, unoptimized version):
...
The scale vector $s$ is found by a small calibration-data grid search (no backprop, no Hessian inversion), making AWQ substantially cheaper than GPTQ while empirically competitive at 4-bit — a good default when calibration compute is tight, which is exactly our situation on a single rented GPU.

```python
def awq_find_channel_scales(W: torch.Tensor, X_calib: torch.Tensor, n_grid: int = 20) -> torch.Tensor:
    """Sketch of AWQ's salient-channel protection (no backprop; grid search over
    a single global strength alpha, per the paper's simplification).
    W: (d_out, d_in). X_calib: (n_samples, d_in)."""
    act_scale = X_calib.abs().mean(dim=0)              # per-input-channel saliency
    act_scale = act_scale / act_scale.mean()            # normalize

    best_scale, best_err = None, float("inf")
    for i in range(1, n_grid + 1):
        alpha = i / n_grid                              # search strength in (0, 1]
        s = act_scale.clamp(min=1e-5).pow(alpha)        # s >= 1 on salient channels
        ...
        # this sketch uses the cheaper weight-space proxy for clarity.
...
class QuantizedLinear(nn.Module):
    ...
    compression buys you — but note it does NOT by itself give a matmul FLOPs
    speedup, since PyTorch still computes in fp32. Real speedups need
    integer-native kernels (Section 9)."""
...
class QuantizedEmbedding(nn.Module):
    ...
    16.78M x 4 B = 67 MB fp32 table alongside the packed weights. That single
    oversight would roughly double every headline in this chapter."""

    def __init__(self, head: QuantizedLinear):
        super().__init__()
        # Deliberately NOT a submodule: nn.Module.__setattr__ would register it,
        # duplicating every key as `tok_emb.head.*` in state_dict(). We want the
...
def _swap_linears(module: nn.Module, bits: int, group_size: int) -> None:
    for name, child in list(module.named_children()):
        if isinstance(child, nn.Linear):
            assert child.in_features % group_size == 0 or bits == 8, (
...
def quantize_stacklm(model: nn.Module, bits: int, group_size: int = 64,
    ...
    `embedding_bits` lets you keep the tied embedding/head at a HIGHER precision
    than the body — e.g. `bits=4, embedding_bits=8`. This is not a hedge: the
    32768 x 512 table is 17% of the parameters and its rare-token rows are
    thinly trained, so it is disproportionately fragile at 4 bits. llama.cpp's
    mixed "K-quant" recipes make the same choice, keeping the output/embedding
    tensor at higher precision than the body. Section 8 lets you measure whether
    it matters for YOUR checkpoint instead of guessing."""
...
>>> quantize_stacklm(m, bits=4, group_size=64)
...
>>> sum(t.numel() * t.element_size() for t in m.state_dict().values()) / 1e6
63.4                                              # MB — matches the table below
```
...
!!! example "Worked example: the memory budget, computed exactly"
    ...
    Stack-100M has 101,353,728 parameters, of which **101,318,656 live in `nn.Linear` weights** (30 blocks × 2,818,048 body params + the 16,777,216-entry tied embedding) and 35,072 in RMSNorm scales that stay fp32. Here is what each format actually costs, computed rather than guessed:
...
def evaluate_quantization_sweep(fp32_model, tok, val_shard_dir, probes, *,
                                configs=((None, None), (8, None), (4, 128), (4, 64)),
                                device="cpu", max_batches=20):
    """`probes` bundles the Section 4 fixtures:
       {"arith": [...], "mc": [...], "qa": [...], "retriever": ..., "env": ...,
        "agent_tasks": [...], "pad_id": int}
    ...
    for bits, gs in configs:
        ...
        # because the constructor performs the tie; a naive `model.clone()`-style
        # helper that copies tensors one by one would NOT.
        m = copy.deepcopy(fp32_model)
        if bits is not None:
            quantize_stacklm(m, bits=bits, group_size=gs or 64)
        m.to(device).eval()

        ppl = compute_perplexity(m, val_shard_dir, probes["pad_id"],
                                 device=device, max_batches=max_batches)
        row = {
            "config": "fp32" if bits is None else f"int{bits}/g{gs or 64}",
            "bytes_mb": sum(t.numel() * t.element_size()
                            for t in m.state_dict().values()) / 1e6,
            "ppl": ppl["perplexity"],
            "arith": eval_arithmetic(m, tok, generate_fn, probes["arith"])["accuracy"],
            ...
            "agent_em": eval_agent(m, tok, probes["env"], probes["agent_tasks"])["exact_match"],
        }
        rows.append(row)
        del m
    ...
    for r in rows:
        ...
        for k in ("arith", "mc", "qa", "agent_em"):
            r["d_" + k] = r[k] - base[k]
    return rows


def print_sweep(rows) -> None:
    hdr = f"{'config':<12}{'MB':>7}{'PPL':>8}{'dPPL':>8}{'arith':>8}{'MC':>7}{'QA':>7}{'agent':>7}"
    print(hdr, "\n", "-" * len(hdr), sep="")
    for r in rows:
        print(f"{r['config']:<12}{r['bytes_mb']:>7.1f}{r['ppl']:>8.2f}{r['d_ppl']:>+8.2f}"
              f"{r['arith']:>8.1%}{r['mc']:>7.1%}{r['qa']:>7.1%}{r['agent_em']:>7.1%}")
```

Fill in this table from your own run — we will not invent the numbers for you:

| Config | Size (MB) | PPL | ΔPPL | Arithmetic | MC (acc_norm) | Retr-QA EM | Agent EM |
|---|---|---|---|---|---|---|---|
| fp32 | ≈405 | *baseline* | 0 | | | | |
| int8 / row | ≈102 | | | | | | |
| int4 / g=128 | ≈57 | | | | | | |
...
- **int8 deltas indistinguishable from noise on every probe.** This is the expected result, and it is why int8 weight-only PTQ is treated as a commodity: 8 bits with a per-row scale reconstructs a well-trained weight matrix to well within the model's own run-to-run variance. If int8 *does* move your numbers, suspect a bug in the packing, not a discovery about quantization.
- **int4 ΔPPL small (a few hundredths of a nat) while a capability probe drops several points.** The central phenomenon of this chapter, and the thing perplexity is structurally incapable of catching (see the Interview Corner). Watch the *arithmetic* and *agent* probes hardest: both require a long chain of exactly-right tokens, so they compound small per-token degradations, whereas perplexity averages them away.
- **g=128 worse than g=64 despite being 6 MB smaller.** The direct empirical version of Exercise 2's arithmetic: wider groups amortize the scale overhead but coarsen the grid. Measuring the tradeoff instead of assuming it is the entire point.
- **Agent EM falling faster than everything else.** Expected, and worth internalizing: a 6-turn trajectory needs ~6 consecutive well-formed emissions, so a per-emission degradation $\epsilon$ compounds roughly as $(1-\epsilon)^6$. Multi-step evaluation is a *magnifier* of quantization damage. Check `tool_format_validity` from §4.4 first — if the grammar itself is degrading, that is your fix (higher-precision embedding via `embedding_bits=8`, or constrained decoding).

Small-$n$ discipline still applies: at $n = 200$ arithmetic items, a 3-point difference is roughly one standard error and means nothing on its own. Report the interval, or run the sweep at three seeds.
...
measured latency and memory. Run: `python -m stacklm.serve.cli --bits 4`
"""
import argparse
import os
import resource
...
from stacklm.tokenizer.bpe import StackTokenizer, SPECIAL_TOKENS   # Ch. 14.3
...
def main():
    ...
    # CPU inference is memory-bandwidth bound at batch 1, so more threads help
    # only up to the point where they saturate DRAM. Sweep this; on many laptops
    # the optimum is the PHYSICAL core count, not the SMT thread count.
    ...
    text = generate_text(model, tok, args.prompt, max_new_tokens=args.max_new_tokens,
                         temperature=0.7, top_p=0.95,
                         allowed_special=frozenset(SPECIAL_TOKENS))
    gen_time = time.perf_counter() - t1

    # Approximate: re-encoding decoded text is not guaranteed to reproduce the
    # generated id count. For exact throughput, have generate_text return ids.
    n_new = max(1, len(tok.encode(text)))
    # Peak RSS is the number that matters — it counts the mmap'd weights and the
    # KV cache, which a Python-level tracer like tracemalloc cannot see at all.
    # (POSIX only; `ru_maxrss` is KiB on Linux and BYTES on macOS — check yours.)
...
!!! example "Worked example: the full laptop memory budget"
    ...
    Put together every piece measured or computed above, for a generation call with a 2048-token context at int4:
    ...
    - **KV cache** (§2, Ch. 14.4's `KVCache`), GQA with `n_kv_heads=2`, `head_dim=64`, 30 layers, bf16: per token per layer, K and V together are $2 \times 2 \times 64 = 256$ elements × 2 B = **512 B**; across 30 layers, $512 \times 30 = 15{,}360$ B ≈ **15 KB/token**; at the full 2048-token context, $15{,}360 \times 2048 \approx$ **31.5 MB**. (Leave the cache in fp32 and it is 63 MB — as big as the weights.)
    - **Transient dequantization buffer**: `QuantizedLinear.forward` materializes one fp32 weight at a time. The largest is the tied head, $32768 \times 512 \times 4\ \text{B} = 67$ MB — *bigger than the entire quantized model*. This is the reference path's real cost, and the reason §7.2's `dequantize_rows` exists for the embedding direction: it gathers only the rows a batch actually needs.
    - **Activations** (single-token decode): a few MB at most.

    Total steady-state: **on the order of 100 MB** of resident memory to hold the entire model plus a full 2048-token conversation, with a transient peak set by the largest dequantized layer. Small enough to run alongside a browser and an IDE on any laptop from the last decade, and small enough for a Raspberry Pi 4. This is the concrete payoff of stacking GQA (4× smaller KV cache than plain MHA), a right-sized 32768-token vocabulary (small embedding table), and int4 weight quantization (6.4× smaller than fp32, scale overhead included) — each individually a modest win, compounding into "runs anywhere."

### 9.1 Why int4 weights don't automatically mean int4 speed — and the libraries that fix it

`QuantizedLinear` dequantizes to fp32 before every matmul. It buys **memory** (RAM and disk), not automatically **compute**, because the matmul still runs in fp32 — and it actually *adds* work per forward (the unpack). At batch size 1 that trade is usually still a win, because single-stream decode is memory-bandwidth bound, not FLOP bound: the arithmetic intensity of a matrix–vector product is ~1 MAC per weight loaded, so time is dominated by streaming weights from DRAM and 4-bit weights stream 8× faster than fp32. See [The Roofline Model & Performance Engineering](../04-kernels-efficiency/01-roofline-performance.html) for why that is a general property of decode, not a quirk of this model. But you only realize the win with a kernel that unpacks *inside* the matmul loop instead of materializing a dequantized weight first. Ours does not. These do:
...
`quantize_` mutates the model in place, swapping each `nn.Linear` weight for a quantized *tensor subclass* that intercepts `F.linear` and dispatches to a packed kernel — conceptually the same swap as our `quantize_stacklm`, done at the tensor level rather than the module level, which is why it composes with `torch.compile`, FSDP, and the rest of the stack. Two caveats worth checking against your installed version rather than assuming: older torchao releases spell these configs as the factory functions `int8_weight_only()` / `int4_weight_only(group_size=64)`, and int4 kernel coverage differs by backend and layout (the int4 path was CUDA-first; CPU support depends on the layout your build ships). Microbenchmark before you believe a speedup.
...
!!! warning "Common pitfall: converting a not-quite-Llama architecture to GGUF"
    ...
    Stack-100M is Llama-shaped *except* for two things: QK-norm, and NoPE on every 4th layer. `convert_hf_to_gguf.py` maps a declared architecture to a fixed graph, so converting it *as* Llama would silently apply RoPE to the 7 NoPE layers and drop the QK-norm weights. Nothing errors; the model just generates degraded text, and you will spend a day blaming the quantizer. The honest routes are (a) add a small architecture entry describing the per-layer RoPE mask and the QK-norm tensors, or (b) accept the mismatch only after verifying token-level logit agreement against the PyTorch reference on a fixed prompt. **Always diff logits against the reference implementation after any format conversion** — the same discipline as Ch. 14.4's cache-equivalence assertion, applied across runtimes.

**Where the GPU stacks fit.** Nothing about `vLLM`, `SGLang`, or `TensorRT-LLM` is wrong for this model — they just solve a problem Stack-100M does not have at batch 1 on a laptop. Their wins are throughput wins: continuous batching, PagedAttention, prefix-cache reuse across requests ([vLLM: Architecture, PagedAttention & Internals](../07-inference-serving/03-vllm-internals.html), [SGLang: RadixAttention & Structured Programs](../07-inference-serving/04-sglang-radixattention.html)). The moment you serve the agent from §4.4 to more than one concurrent user — where every request shares the same long system prompt and tool schemas — prefix caching alone is worth more than everything in this section. vLLM loads GPTQ, AWQ, and compressed-tensors checkpoints directly, which is why the llm-compressor route above is the one that composes with production serving. And `bitsandbytes` belongs to a different problem again: its NF4 path is the *training*-time 4-bit quantization behind QLoRA, not a serving format.

Measure your own `tok/s` with the CLI above before and after each of these. The gap is the lesson.
...
!!! interview "Interview Corner"
    ...
    **A:** 40% end-to-end is not actionable — decompose it. (1) **Tool-call format validity**: what fraction of emissions parse as a well-formed call? If that is low, the model never learned the grammar, and the cheapest fixes are constrained/grammar-guided decoding at sampling time and more distillation traces — not a bigger model. (2) **Tool-choice accuracy** against a gold first tool: high format validity with low choice accuracy means the model learned the *syntax* and not the *policy*, which is a data-coverage problem (your traces are unbalanced across tools). (3) **Turns and non-termination rate**: if it is burning the step cap, the harness guards matter more than the model — repeated-call detection and forced synthesis convert hangs into recoverable observations. (4) **A failure taxonomy over the traces**: if `retrieved_but_ignored` dominates, retrieval is fine and extraction is the bottleneck, so target SFT on grounded-answer formatting; if `wrong_tool` dominates, target the routing decision. (5) Finally, **confidence intervals**: at n=30, 40% has a 95% Wilson interval of roughly 25–58%, so before acting on any of the above, check whether the differences you are chasing are larger than the noise. In production this decomposition is what frameworks like `inspect_ai` give you for free — per-step tool records, turn accounting, and per-metric scorers over a logged trace.
...
Before that: run `python -m stacklm.serve.cli --bits 4` on your own laptop, on your own checkpoint, and read what it wrote. That sentence is the actual point of the entire capstone — not the perplexity number, not the tok/s number, but the fact that a model you trained end to end, on hardware you rented for less than a dinner out, just generated a sentence on the machine in front of you.

!!! key "Key Takeaways"

    - Evaluation and sampling need *opposite* settings of the same knob: `logits_to_keep=1` is a prefill optimization for generation, `logits_to_keep=0` (all positions) is a hard requirement for perplexity and cloze scoring. Copy a sampling call into an eval function unchanged and you silently score one position. And pass `position_ids`/`seq_ids` through — evaluating without the document geometry the model trained in measures a different model.
    - Ch. 14.4's cache-invariance assertion (`generate(..., use_cache=True) == generate(..., use_cache=False)` under greedy decoding) proves the *fp32* model's decode path. It does not transfer across a model edit: re-run it after quantization, and diff logits against the PyTorch reference after any format conversion (GGUF, ONNX, TensorRT). Cache and conversion bugs produce fluent, wrong text, not crashes.
    - A model is not "done" at a loss curve. It is done at a defensible number (held-out perplexity, provably excluded from every training stage), a set of cheap probes (arithmetic EM, cloze-scored MC reporting both `acc` and `acc_norm`, retrieval-QA EM *with retriever recall reported separately*), an **agent-loop evaluation** decomposed into format validity / tool choice / turns / EM / failure taxonomy, and an honest statement of what the model cannot do.
    - Perplexity measures predictive fit to held-out text and nothing else; it is not comparable across tokenizers (use bits-per-byte for that), and it can stay flat while a capability collapses — multi-step evals *magnify* damage that averaging hides, roughly as $(1-\epsilon)^k$ over $k$ steps.
    - Contamination is real even for hand-built probes, and the nastiest leak is self-inflicted: hold your agent eval tasks out of the Ch. 14.10 distillation rollout by hash, or you are measuring memorized trajectories.
    - RTN quantization is a fast, calibration-free baseline; GPTQ (Frantar et al., 2022) redistributes each column's error onto not-yet-quantized columns via the layer's Hessian; AWQ (Lin et al., 2023) protects high-activation salient channels by rescaling and folding the inverse scale into the preceding norm. Both beat RTN at 4 bits exactly where RTN is weakest.
    - **Tied embeddings break naively.** Replacing `lm_head` with a quantized module severs the alias to `tok_emb`, leaving a 67 MB fp32 table in the export and roughly doubling the checkpoint. Detect the tie by `data_ptr()`, replace the embedding with a row-gathering view over the quantized head's buffers, and assert the byte count of the actual `state_dict`.
    - Exact accounting for Stack-100M's 101.32M quantizable parameters: fp32 ≈405MB → bf16 ≈203MB → int8 ≈102MB (4.0×) → int4/g64 ≈63MB (6.4×, counting the 12.66MB of fp32 scales/zero-points honestly — 20% of the total, which is why "int4 = 8×" is a lie).
    - int4 weights shrink memory, not FLOPs, on a dequantize-then-matmul path. Real throughput needs integer-native kernels: **torchao** (`quantize_(model, Int4WeightOnlyConfig(...))` + `torch.compile`) is PyTorch's supported path, **llm-compressor**/**compressed-tensors** is the GPTQ/AWQ pipeline vLLM reads natively, and **GGUF + llama.cpp** is the fastest CPU/edge route — with the caveat that converting a not-quite-Llama architecture silently corrupts NoPE and QK-norm unless you diff logits against the reference.
    - GQA's 4× smaller KV cache (15 KB/token, 31.5 MB at 2048 in bf16), a right-sized 32768-token vocabulary, and int4 weights compound: model plus a full 2048-token conversation fits in roughly 100MB of RAM.
    - The payoff of the entire capstone is checkable in one command: `python -m stacklm.serve.cli --bits 4`, and read text your own model produced on your own machine.
...
**3.** Work RTN by hand. Take a single group of four weights $w = [\,0.90,\ -0.40,\ 0.10,\ -0.85\,]$ and apply the **$b=4$ symmetric** scheme from §6.1 (so $2^{b-1}-1 = 7$). (a) Compute the scale $s$. (b) Compute the integer codes $q$ and the dequantized weights $\hat w$. (c) Report the maximum absolute reconstruction error.
...
??? note "Solution"
    ...
    (c) Per-element errors $w_i - \hat w_i$: $0.000,\ -0.0143,\ -0.0286,\ +0.0500$. The **maximum absolute error is $0.050$** (on the $-0.85$ weight). Note the largest error falls on $-0.85$, whose scaled value $-6.61$ lands near a rounding boundary, not on the largest-magnitude weight $0.90$, which sits exactly on the grid endpoint with zero error — a small illustration of RTN's blind spot from §6.1: it rounds every weight independently by grid proximity alone, with no notion of which one matters more to the layer's output.
...
??? note "Solution"
    ...
    (This drops into `QuantizedLinear.from_float`/`dequantize`: for `bits == 4` you register `q_weight` and `scale` only, with no `zero_point` buffer — and remember to update `QuantizedLinear.empty` to match, or `load_state_dict` will fail on a missing key.)
...
**7.** (Quantitative) Stack-100M's KV cache costs 15,360 B/token in bf16 (§2). (a) At what context length does the bf16 KV cache exceed the ≈63.3 MB int4 weight budget, and what is the answer if you leave the cache in fp32? (b) After mid-training extends the context to 8192 (Ch. 14.8), what does the bf16 cache cost, and what does that do to the "runs on a Raspberry Pi" claim? (c) Ch. 14.4's CI asserts that greedy generation is identical with and without the cache. Explain precisely why that assertion must be re-run against the *quantized* model rather than inherited from the fp32 one, and name the specific `QuantizedLinear` bug it would catch.
...
??? note "Solution"
    ...
    (b) $15{,}360 \times 8192 = 125{,}829{,}120$ B $\approx 126$ MB — **twice** the int4 weights. The "≈100 MB total, fits on a Pi" figure in §9 is stated for a 2048-token context and does not survive a full 8192-token conversation; the honest number there is ≈190 MB (63 MB weights + 126 MB cache), which still fits a 512MB-class device but no longer leaves the comfortable margin. Report the context length alongside any memory claim, or the claim is not falsifiable.

    (c) The assertion is a property of *the modules the decode path executes*, not of the architecture. Section 7 replaces every `nn.Linear` with a `QuantizedLinear`, so the fp32 proof covers code that is no longer running. The specific bug it catches is a **shape-dependent** dequantization error: `dequantize_int4_grouped` reconstructs `q[:, 0::2], q[:, 1::2]` from the low/high nibbles, and any nibble-order or `view(d_out, n_groups, group_size)` mistake produces a *deterministic* wrong weight. That wrong weight is applied identically on both paths, so cache-invariance alone would **not** catch it — which is why the test to run is the pair: (i) cache-invariance on the quantized model, and (ii) a logit diff of the quantized model against `dequantize()`-then-`nn.Linear` on the same input. (i) catches cache/masking regressions introduced by the module swap; (ii) catches packing errors. Neither alone is sufficient, and this is exactly the "diff against the reference after every model edit" discipline §9 applies to GGUF conversion.
...
??? note "Solution"
    ...
    (c) Check **`tool_format_validity`** first, because a 13-point EM drop against a 0.03-nat PPL change is exactly the "perplexity is flat, capability collapsed" signature, and multi-step trajectories magnify per-token degradation as $(1-\epsilon)^k$. If format validity has fallen (say 0.97 → 0.80), the quantization damaged the model's grip on the rare special-token grammar — the fix is precision where it matters (`embedding_bits=8` for the tied table, or GPTQ/AWQ instead of RTN) or constrained decoding, which makes malformed output impossible by construction. If format validity is *unchanged* at 0.97, the grammar survived and the damage is in the policy/extraction — expect `retrieved_but_ignored` and `wrong_tool` to have grown, which points at reconstruction-aware quantization (GPTQ) rather than at the decoder.
