...
For most of the last decade, "running an LLM" silently meant "running it on an NVIDIA GPU with CUDA." That assumption is no longer safe, and a working LLM engineer who only knows CUDA is now under-equipped. Some of the largest production language models on earth — Google's Gemini family — are trained on **TPUs**, not on NVIDIA GPUs (Google does also serve Gemini on NVIDIA Blackwell systems for on-prem/sovereign deployments, but the training silicon is its own). Anthropic and many AWS customers train and serve on **Trainium** and **Inferentia**. AMD's **Instinct MI300X**, with 192 GB of HBM3 per package, has become a credible serving target precisely because its memory capacity lets a 70B-class model fit on fewer devices. Intel's **Gaudi** is the budget challenger. Procurement reality — supply, price, and the simple fact that you cannot buy as many top-tier NVIDIA GPUs as you want — means you *will* eventually be handed a non-NVIDIA fleet and asked to make it sing.
...
def pick(workload, params_b, dtype_bytes, L, H_kv, d_h,
    ...
    for c in CHIPS:
        ...
        # so BOTH scores secretly reward SMALL memory (a chip with half the
        # HBM needs 2x the devices and doubles its aggregate). For DECODE
        # that is backwards, because splitting the model is exactly the
        # per-layer all-reduce you wanted to avoid: below, 2x Gaudi3 scores
        # 7.4 TB/s against the single MI300X's 5.3. Exercise 5 rewrites this
        # to rank decode by FEWEST DEVICES first. An aggregate ranking is
        # only meaningful at a FIXED device budget; compare per-device (or
        # per-dollar-hour) figures instead, using n_dev as the feasibility
        # gate only.
...
The function gates on capacity first — a chip that cannot hold the model is disqualified before any throughput comparison — and then scores the survivors on the figure that matches the regime: HBM bandwidth for **decode**, low-precision TFLOPS for **training/prefill**. Read the printed scores with the caveat in the code, though, because the toy multiplies a per-device figure by `n_dev` and an *aggregate* rewards needing more devices. That artifact bites the decode column too: on the 70B footprint below it ranks two Gaudi3s ($2 \times 3.7 = 7.4$ TB/s) above the single MI300X (5.3 TB/s), even though splitting the model across two devices reintroduces the per-layer all-reduce decode most wants to avoid. The doctrine the chapter actually recommends for decode is therefore *fewest devices that fit, then highest bandwidth*; Exercise 5 rewrites `pick()` to implement exactly that and to return a winner instead of only printing.
...
!!! example "Worked example: serving a 70B model — capacity changes everything"
    ...
    - On a **192 GB MI300X**, the whole 156 GB fits on **one** device, with ~36 GB left for activations, workspace and fragmentation. No tensor-parallel collective in the decode path at all. Because decode is **bandwidth-bound**, that single MI300X streaming weights at $\sim 5.3\ \text{TB/s}$ from one HBM stack is a genuinely strong serving configuration despite the MI300X's raw FLOPS being a serving afterthought.

    Be precise about *why* it is strong, because the tempting argument is the wrong one. Two H100s at TP=2 have $2 \times 3.35 = 6.7\ \text{TB/s}$ of aggregate bandwidth and each streams only *half* the weights per token, so on raw decode throughput a well-provisioned H100 tensor-parallel group is ahead of one MI300X — and the all-reduce you eliminate is small in decode (a few hundred KB per collective at batch 32), a single-digit-percent tax rather than a throughput inversion. The MI300X's real advantages are that it *fits at all* (156 GB into $2 \times 80$ GB leaves ~4 GB for everything else, so in practice you would provision three H100s), that one device serves the model at lower cost per token and with far simpler operations, and that the spare capacity buys larger batches and longer contexts. That — capacity, cost and simplicity, not a bandwidth inversion — is the honest form of "the memory-rich chip wins decode."
...
Run the same three-number reasoning on this book's capstone model ([The Capstone: Building Stack-100M, and the 2026 Small-Model Landscape](../14-capstone/01-overview-and-landscape.html)) and the answer inverts the 70B story. At $N \approx 100\times10^{6}$ parameters trained in mixed precision under AdamW, the persistent state is $100\times10^{6}\times(2 + 2 + 4 + 4 + 4) \approx 1.6\ \text{GB}$ — bf16 weights, the bf16 gradient buffer that is live from backward through the optimizer step, the fp32 master copy, and Adam's two fp32 moments, i.e. the standard 16 B/param (the capstone's actual Muon+AdamW mix carries a single momentum buffer on the 2D hidden matrices and so lands nearer $1.28\ \text{GB}$); activations at a modest batch and 2k context add a few GB more. The entire training job therefore fits on **one** 16 GB accelerator with room to spare, which is exactly why [The Pretraining Run: A Complete Single-GPU Training Loop](../14-capstone/07-pretraining-run.html) is a single-device loop with no tensor or pipeline parallelism at all. At this scale you are never capacity-bound — you are **throughput**-bound, and the figure to maximize is achieved bf16 TFLOP/s per dollar-hour, because the ~20B-token budget of [Data: Sourcing, Filtering, Dedup, Tokenize & Pack ~20B Tokens](../14-capstone/02-data-pipeline.html) is what sets wall-clock. Practically: a free Colab T4 works but is slow *and* has no bf16 tensor cores (it is a Turing part, so the loop must fall back to fp16 autocast plus a `GradScaler` — see [The Pretraining Run: A Complete Single-GPU Training Loop](../14-capstone/07-pretraining-run.html)); a single A100/H100 hour, one MI300X, or a free Colab/Kaggle TPU slice (via JAX or `torch_xla`) all clear the bar, and the ROCm and XLA paths above are genuinely usable for the capstone rather than being NVIDIA-only. Serving the finished model is the opposite extreme again: 100M int4 weights are roughly 50 MB, so it runs on a laptop CPU through llama.cpp — see [Evaluation & Serving: Honest Benchmarks, int4 Quantization, and Running on a Laptop](../14-capstone/11-evaluation-and-serving.html).
...
!!! interview "Interview Corner"
    ...
    **A:** Because autoregressive **decode is memory-bandwidth-bound, not compute-bound.** Each generated token requires streaming the full set of weights (and the KV cache) from HBM while doing only a tiny amount of arithmetic, so the binding constraint is HBM bandwidth and *capacity*, not peak matmul FLOPS. Fitting the entire model plus KV cache in the MI300X's 192 GB on a **single device** eliminates the per-layer tensor-parallel all-reduce that two H100s would pay over NVLink, and it streams weights from one HBM stack at very high bandwidth. I would be careful about *which* part of that argument does the work, though: two H100s have more aggregate bandwidth ($2 \times 3.35 = 6.7$ vs $5.3$ TB/s) and the eliminated all-reduce is only a single-digit-percent tax in decode, so the MI300X's edge is not raw throughput — it is that 156 GB barely fits in $2 \times 80$ GB at all (you would realistically buy a third H100), and that one device means lower cost per token, no TP plumbing, and headroom for larger batches and longer contexts. I would **reverse** the decision when the workload becomes **compute-bound**: long-context prefill on large batches, or training/fine-tuning, are dense high-arithmetic-intensity GEMMs where aggregate FP8/bf16 TFLOPS and interconnect all-reduce efficiency dominate — there the two-H100 (or a larger NVLink/TPU-pod) configuration with more compute and a mature kernel stack typically wins.
...
!!! sota "State of the Art & Resources (2026)"
    ...
    **Recent advances (2023–2026)**
    ...
    - [Ambati & Diep, *AMD MI300X GPU Performance Analysis* (2025)](https://arxiv.org/abs/2510.27583) — systematic microbenchmark study of MI300X compute, memory bandwidth (~81% of 5.3 TB/s peak), and Infinity Fabric interconnect, directly informing serving decisions.
...
!!! key "Key Takeaways"
    ...
    - **Decode serving is bandwidth-bound; training/prefill is compute-bound.** This single distinction flips which chip wins — which is why a single 192 GB MI300X is a credible decode server against a two-H100 tensor-parallel pair (it fits the whole model, costs less per token, and needs no collectives) while losing badly on training. Note the honest form: two H100s still hold more *aggregate* bandwidth, so it is capacity, cost and simplicity that put the MI300X ahead, not a bandwidth inversion.
