...
A **Tensor Core** is a small systolic-array-like unit that computes an entire small matrix multiply-accumulate per operation: $D = A \times B + C$ where $A$, $B$, $C$, $D$ are small tiles (e.g. $16\times16$). Instead of issuing $16^3$ scalar FMAs, one Tensor Core instruction consumes whole tiles and produces a whole tile. This is why the BF16/FP16 Tensor Core throughput of an A100 (on the order of 312 TFLOP/s) is **16×** its FP32 CUDA-core throughput. The H100 pushes this further with FP8 Tensor Cores; the Blackwell B200 adds FP4. Every modern training and inference workload lives or dies on Tensor Core utilization.
...
| PCIe (to host) | node | — | ~32–64 GB/s | microseconds |
...
!!! example "Worked example: occupancy as a resource-packing problem"
    ...
    Want higher occupancy? Cut shared memory per block to 16 KB and now $\lfloor 100000/16000 \rfloor = 6$ blocks fit, but registers now bind at 5 blocks → $5\times256=1280$ threads = 20 warps... wait, 5 blocks of 256 = 1280 threads = 40 warps = **62.5% occupancy**. Drop registers to 32/thread and the register limit becomes $\lfloor 65536/(256\times32)\rfloor = 8$ blocks, which hits the thread cap at 8 blocks = 2,048 threads = **100% occupancy**. This is the daily reality of kernel tuning: you trade registers and shared memory against occupancy.
