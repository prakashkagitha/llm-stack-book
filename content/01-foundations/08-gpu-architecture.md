...
- **Blackwell Ultra and beyond (2025–2026).** The B200 generation was refreshed as **Blackwell Ultra (B300/GB300)** — same Blackwell architecture family and HBM3e generation, with more memory and FP4 throughput per GPU — and by 2026 it is the GPU behind NVIDIA's flagship GB300 NVL72 racks. NVIDIA's next architecture, **Rubin**, is slated to ship in the second half of 2026 with **HBM4** memory and a roadmap claim of roughly 2.5× Blackwell's FP4 throughput; as always, treat pre-launch vendor figures as directional until independently benchmarked. The trend from the last four generations — compute and low-precision throughput outrunning bandwidth — shows no sign of reversing.
...
!!! sota "State of the Art & Resources (2026)"
    GPU architecture for LLM workloads is defined today by the Hopper/Blackwell lineage — now including the Blackwell Ultra (B300/GB300) refresh, with NVIDIA's HBM4-based Rubin architecture slated for the second half of 2026: widening HBM bandwidth, FP8/FP4 Tensor Cores, and hardware-managed async copies (TMA) that let kernels overlap compute and data movement. The central tension — compute growing faster than bandwidth, raising the ridge point each generation — continues to make memory-hierarchy awareness the dominant skill in ML systems engineering.
