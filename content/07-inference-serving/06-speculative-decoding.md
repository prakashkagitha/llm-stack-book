...
def generate(prompt, max_new_tokens=128, gamma=4, temperature=1.0):
    ...
    # A step commits 1..gamma+1 tokens at once, so the loop OVERSHOOTS by up to
    # gamma. Truncate to exactly max_new_tokens so the output is comparable
    # token-for-token with `target.generate(..., max_new_tokens=...)`.
    return tok.decode(ids[0, start_len:start_len + max_new_tokens],
...
- **KV-cache reuse.** Re-running `draft(draft_ids)` and `target(draft_ids)` from scratch each step is wasteful. Real implementations pass cached KV and only feed the new tokens, so the drafter's steps each see one new position (its *first* step also has to catch the cache up on the committed token(s) not yet cached) and the target's verify pass sees $\gamma+1$ new positions — the pending committed token plus $d_1,\dots,d_\gamma$, since the target's distribution *at* the last committed token is exactly what the accept test for $d_1$ needs. After a step, both caches must be **rolled back** to the accepted length — the rejected/unused positions' KV entries are evicted. See [PagedAttention & KV-Cache Memory Management](../04-kernels-efficiency/06-paged-attention-kv.html) for how paged caches make this cheap.
- **Numerical stability.** Compute $p/q$ carefully; clamp $q$ away from zero. The hazard lives at temperature > 0, where $q(d)$ for an unlikely drafted token can underflow. At temperature 0 there is no division to worry about: the drafted token is $q$'s argmax and $q(d)=1$ exactly, so the ratio collapses to a 0-or-1 indicator — just compare argmaxes directly (and never literally divide the logits by a temperature of 0).
...
The `speculative_step` above re-runs `draft(draft_ids)` and `target(draft_ids)` over the *entire* sequence every single step — an $O(T)$ recompute that gets more wasteful the longer the generation runs. A production implementation instead keeps a KV cache for each model and feeds only the *new* tokens at every step, then **rolls each cache back** to the accepted length with `DynamicCache.crop(...)` once accept/reject is resolved. The invariant to hold in your head: at the start of a step, each cache holds KV for the committed prefix *except* the final committed token, which is left "pending" and re-fed as the first input of the next step. For the **target** cache this is exact — `target_cache.get_seq_length()` is always precisely one less than the committed length. The **draft** cache trails by *one or two*: two right after an all-accepted step, because the last draft token $d_\gamma$ is never fed back to the drafter (the note in step 4 of the listing below works through why). That is why each step's first move is to feed the whole uncached tail, `prefix_ids[:, cache.get_seq_length():]` — a slice, not a hard-coded `[:, -1:]`, which would silently skip a token and corrupt the draft cache. This is exactly the bookkeeping Hugging Face's assisted-generation implementation performs internally.
...
The one number to watch in production is the **acceptance rate**, which every engine reports: vLLM exposes Prometheus counters for drafted and accepted speculative tokens, and SGLang logs an average accept length. Read those counters carefully. Every step drafts all $\gamma$ tokens but stops accepting at the first rejection, so the aggregate ratio accepted/drafted is $\tfrac{1}{\gamma}\sum_{i=1}^{\gamma}\alpha^i$, which is *below* the per-position $\alpha$ of the next section (at $\alpha=0.8,\ \gamma=5$ the ratio is $0.54$, not $0.8$) — recover the per-position $\alpha$ from vLLM's per-position accepted-token counter, which breaks acceptance out by draft slot. Likewise, accepted *draft* tokens per step is $\mathbb{E}[\text{tokens per step}] - 1$, while SGLang's accept length already counts the bonus/corrected token. If the accept length drifts toward 1 (essentially no drafts accepted), speculation is pure overhead and should be switched off — which is exactly what the economics below quantify.
