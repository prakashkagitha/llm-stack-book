...
- **`value`** is *not* the KV tensors themselves; it is a tensor of **indices** into the global paged KV pool (the `token_to_kv_pool_allocator`). The radix tree is a thin index structure layered on top of the same paged memory you met in [PagedAttention & KV-Cache Memory Management](../04-kernels-efficiency/06-paged-attention-kv.html). This separation is what lets many concurrent *requests* point at the same node's KV blocks without copying a single tensor, and lets one sequence's KV live in non-contiguous pages. (Each slot index still has exactly one owning node — `_split_node` *partitions* the index tensor between the new parent and the old child rather than duplicating references — which is what makes it safe for `evict` to `free(node.value)` when a leaf goes.)
...
??? note "Solution"
    ...
    # re-promoted onto the heap it would sort at the TOP of the min-heap and be
    # evicted first -- the opposite of what LFU wants.
