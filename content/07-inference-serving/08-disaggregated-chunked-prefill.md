...
For a 70B parameter model in BF16 (140 GB of weights), each decode step reads roughly 140 GB of weights from HBM regardless of batch size — plus the KV cache of every sequence in the batch, which unlike the weights *does* grow with batch size and context length, and eventually dominates at long context. Read that as an *aggregate* bandwidth cost, and divide by the shard count: 140 GB does not fit on one 80 GB A100 or H100, so every real deployment shards the model and the aggregate bandwidth scales with the tensor-parallel degree. At roughly 2 TB/s per A100 SXM, 140 GB is 70 ms of bandwidth time against one device's worth of bandwidth, so an 8-way tensor-parallel A100 group ($\approx 16$ TB/s aggregate) needs about 9 ms per step; eight H100s (3.35 TB/s each) about 5 ms; a pair of 2026-frontier Blackwell B200s (8 TB/s HBM3e, 180 GB each) again about 9 ms. Whatever the shard count, that time is nearly all bandwidth — leaving essentially no room for compute to hide, and the decode phase stays bandwidth-bound on every generation of hardware.
...
!!! example "Worked Example: Chunked Prefill Latency"
    ...
    **Without chunked prefill:** The prefill runs as one monolithic forward pass. Empirically, a 13B model prefill over 8K tokens on an H100 takes roughly 800 ms (the work is $2 \times 13\text{B} \times 8192 \approx 2.1 \times 10^{14}$ FLOPs of GEMM plus $\approx 2.7 \times 10^{13}$ of *causal* attention (the $4 L d_\text{model} T_p^2$ full-matrix count halved, since a causal FlashAttention kernel skips the masked blocks), so 800 ms is about 30% MFU against the H100's 989 TFLOP/s BF16 peak — a realistic, slightly conservative figure). During this time, all 32 decode sequences are stalled — their ITL spikes by 800 ms. If the SLO is P99 ITL ≤ 100 ms, this is an 8x violation.
...
3. **Implemented direct GPU-to-GPU KV transfer** together with a bandwidth-aware *placement* algorithm: because the decode instance pulls the cache from the prefill instance's GPU memory, DistServe's placement search deliberately colocates the paired prefill and decode segments of a model on the same NVLINK node, so the transfer rides a high-bandwidth intra-node link and becomes a negligible fraction of end-to-end latency. The near-zero overhead comes from *where the pair is placed*, not from overlapping the transfer with compute.
...
Splitwise is also the source of the **layer-wise KV transfer** described earlier in this chapter: rather than shipping the cache in bulk at the end of the prompt phase, it streams each layer's KV as soon as that layer's prompt computation finishes, hiding the transfer under the remaining layers' compute. Splitwise also introduced the term **"prompt phase"** for prefill and **"token phase"** for decode, now widely adopted in the systems literature. Their key empirical finding: on commercial cloud deployments, the decode phase issues roughly the *same* FLOPs per token as prefill (a dense forward pass is $\approx 2N$ FLOPs per token either way) but achieves a far lower fraction of peak FLOP/s, so it consumes a disproportionate share of total serving cost while sitting idle waiting on memory bandwidth. Disaggregation with heterogeneous hardware can reduce per-token cost by routing each phase to its best-fit hardware.
