...
- **Prefill-prioritizing (decode-blocking), classic vLLM V0 default (and TGI's `--waiting-served-ratio` behaviour).** When a prompt is ready, run its prefill as a dedicated iteration (or admit it ahead of decodes). Maximizes prefill throughput and minimizes TTFT, but causes the decode stutter above.
...
| `num_preemptions_total` climbing steadily | Thrashing: admitted past the KV headroom | Lower `max_num_seqs`, shorten `max_model_len`, or *raise* `gpu_memory_utilization` to give the KV pool more blocks |
