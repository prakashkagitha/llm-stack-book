...
```python
# real_ep_alltoall.py — the actual collective. NOTE: in practice this needs GPUs
# and the NCCL backend: the Gloo backend has no all-to-all at all (only the
# rarely-built MPI backend also implements it). Run with:
...
!!! example "Worked example: sizing a large-EP DeepSeek-V3 decode deployment"
    ...
    **KV budget.** MLA compresses KV to a small latent: DeepSeek-V3 caches $\text{kv\_lora\_rank}+\text{qk\_rope\_head\_dim} = 512+64 = 576$ elements per token per layer, so $576\cdot61 \approx 35\text{K}$ elements/token across all layers — **~70 KB/token in BF16, ~35 KB/token in FP8**, versus megabytes for vanilla MHA at this head count. Of the ~22 GB/GPU free, budget ~5 GB for activations, workspace, CUDA-graph pools and DeepEP's pre-allocated communication buffers, leaving ~17 GB for KV: with an FP8 KV cache one GPU holds $\sim 17\text{e}9 / 35\text{e}3 \approx 485{,}000$ tokens of KV — e.g. ~14 concurrent 32K-context requests per GPU, ~230 across the 16-GPU pool. KV capacity is comfortable precisely because MLA + attention-DP avoids replication.
...
!!! sota "State of the Art & Resources (2026)"
    ...
    **Recent advances (2023–2026)**
    ...
    - [Gale et al., *MegaBlocks: Efficient Sparse Training with Mixture-of-Experts* (2022)](https://arxiv.org/abs/2211.15841) — block-sparse GPU kernels for MoE that eliminate token dropping and padding waste; the reference block-sparse MoE kernels, widely used in *training* stacks (Megatron-LM, LLM Foundry) and the conceptual ancestor of the grouped-GEMM kernels serving engines use today (vLLM/SGLang's Triton fused-MoE and CUTLASS grouped GEMM, DeepGEMM).
...
- Aminabadi et al., *DeepSpeed-Inference*, SC 2022, and Rajbhandari et al., *ZeRO-Infinity*, SC 2021 — the offload hierarchy (HBM/CPU/NVMe) underpinning expert and KV offloading.
...
??? note "Solution"
    ...
    Implication for offloading: offloading (keep hot params in HBM, stream cold experts from CPU/NVMe) relies on the per-step expert *set* being small. That holds only at small batch / low $k$ / low QPS. At the high concurrency that MoE needs to be efficient, a batch touches most experts every step, so you would be streaming nearly the whole expert set over PCIe (~64 GB/s) every step. At DeepSeek-V3 scale that is $\approx 11$ GB of routed experts per MoE layer ($256\times 3\cdot 7168\cdot 2048$ params in FP8) and $\approx 650$ GB per decode step across 58 MoE layers — $\approx 175$ ms per layer and $\approx 10$ s per step, which no amount of overlap can hide. Offloading therefore buys *capacity* for low-QPS or offline-batch workloads, not low latency at high concurrency; for interactive high-throughput serving you must keep experts in HBM.
