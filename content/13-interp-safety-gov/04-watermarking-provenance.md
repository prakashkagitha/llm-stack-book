...
if __name__ == "__main__":
    ...
    # At T=200 with h=1 the signal decays as (1-p)^2, so a 40% substitution
    # already drops z below the z*=4.0 threshold. Longer passages tolerate
    # more, because z_0 grows only as sqrt(T) (see prose below).
```
...
Here the 40% substitution attack already drops $z$ below the 4.0 threshold — and not just for this draw: averaged over random substitutions the mean $z$ at $p=0.4$ is $\approx 3.7$ (Exercise 5). Two things make the toy this fragile. First, each substitution damages *two* positions, because this implementation seeds the green list from the previous token ($h=1$): the signal decays as $(1-p)^2$, not $(1-p)$. Second, $T = 200$ is a short passage, and the substitution rate a watermark can tolerate is a property of *length*, not of the model. The unattacked score grows only as $z_0 \propto \sqrt{T}$ while the attack multiplies it by $(1-p)^{h+1}$, so solving $z_0 (1-p)^2 = 4$ with this demo's $z_0 \approx 11.5\sqrt{T/200}$ puts the crossover near $p \approx 0.4$ at $T=200$, $p \approx 0.6$ at $T=1000$, and $p \approx 0.7$ only past a few thousand scored tokens. Do not expect a real LM to rescue a short passage: at the same $\delta$ and $\gamma$ its green fraction is typically *lower* than this toy's $181/200$, because a $\delta = 2$ boost frequently cannot outvote a confident argmax, and low-entropy positions (see "Practical Limitations" below) carry no signal at all. Real-world robustness comes from scoring more tokens — and from the fact that a paraphrase preserves far more of the original tokens and contexts than the uniform random replacement simulated here, so its *effective* $p$ is well below the fraction of surface text it changes.
...
**In the serving stack.** A watermark is just a logits processor, so it belongs at the same point in the pipeline as top-p and repetition penalty — see [Sampling Strategies & Decoding Algorithms](../07-inference-serving/09-sampling-decoding.html). vLLM exposes a logits-processor extension point for exactly this, though the interface changed with the V1 engine (per-request `logits_processors` callables in the older engine; a batched `LogitsProcessor` class registered as a plugin in V1), so pin your version and check its docs before writing the hook. The ordering rule is version-independent and easy to get wrong: **apply the watermark bias after temperature and truncation warpers**, which is what `transformers` does (it appends the watermark processor last; see its issue #34630). Be clear about *why*, because the intuitive reason is backwards. Going last does not protect promoted green tokens from truncation — it is precisely what leaves an out-of-nucleus green token pruned, since top-$k$/top-$p$ have already set it to $-\infty$ and the processor only does `scores[green] += bias`, and $-\infty + \delta = -\infty$. Going last is right because it keeps $\delta$ on the scale you configured (a temperature warper applied *after* the bias would rescale it to $\delta/T$) and because it stops the watermark from dragging tokens the model judged implausible into the sampled set. The cost is a little detectability: the bias can only reshuffle within the candidates that survived truncation.

If you are building Stack-100M in Part XIV, this is the hook to add to its serving path ([Evaluation & Serving: Honest Benchmarks, int4 Quantization, and Running on a Laptop](../14-capstone/11-evaluation-and-serving.html)). Do measure before you trust it: a 100M-parameter model produces shorter and far more repetitive text than a frontier model, and repeated-$n$-gram deduplication throws away the repeats, so what governs how fast $z$ accumulates is the number of *scored* tokens rather than the raw output length. Empirically calibrate how many tokens you need for $z > 4$ on *your* model rather than importing a number from the paper.
...
!!! key "Key Takeaways"
    ...
    - Substitution robustness scales as $(1-p)^{h+1}$, not $(1-p)$, because each edit also corrupts the context hash of the following $h$ positions: in the chapter's toy $h=1$ simulation a 40% substitution attack cuts $z$ to roughly a *third* (11.5 → 3.7) and clears the threshold. How much substitution you can tolerate is set by passage *length*, not by model quality, since $z_0$ grows only as $\sqrt{T}$: the same scheme crosses the threshold near $p \approx 0.6$ at $T \approx 1000$ scored tokens, and rates above 70% need several thousand.
...
??? note "Solution"
    ...
    What would buy more robustness: **length**, not model quality. The decay exponent $(1-p)^{h+1}$ is fixed by the seeding scheme, and the unattacked score

    $$
    z_0 = \frac{(q-\gamma)\sqrt{T}}{\sqrt{\gamma(1-\gamma)}} \approx 0.81\sqrt{T}
    $$

    (using this demo's $q \approx 0.905$, $\gamma = 0.5$) grows only as $\sqrt{T}$. Solving $z_0 (1-p)^2 = 4$ gives $p \approx 0.41$ at $T = 200$, $p \approx 0.60$ at $T = 1000$, and $p \approx 0.72$ at $T = 4000$ — so tolerating a 70% uniform random substitution takes *thousands* of scored tokens, and no property of a real LM can change that exponent. If anything a confident, low-entropy model makes things worse at fixed $T$: many positions have one overwhelmingly likely token that a $\delta = 2$ boost cannot outvote, which lowers $q$ and therefore lowers $z_0$ (this is the same "low-entropy text" limitation the chapter discusses). What *does* help in practice is that real attacks are paraphrases rather than uniform random replacement: a paraphrase leaves a large share of the original tokens and their preceding contexts intact, so its effective $p$ is well below the fraction of surface text it rewrites.
