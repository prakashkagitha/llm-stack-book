...
The empirical result is that YaRN extends context further than PI or plain NTK *and* with less fine-tuning — it is the basis of many "32K → 128K" and "128K → 1M" model releases. The lineage is worth memorizing: **PI** (uniform squeeze, needs fine-tune) → **NTK-aware** (non-uniform via base, often training-free) → **YaRN** (per-dimension ramp + logit temperature, the production default). HuggingFace exposes these through a `rope_scaling`/`rope_parameters` config whose `type` is one of `linear`/`dynamic`/`yarn`/`llama3`/`longrope`; the `dynamic` variant recomputes the scale on the fly so short sequences stay un-degraded, and Llama 3.1's `llama3` type applies a frequency-split rescaling (separate low- and high-frequency factors) that has itself become a common default.
...
!!! sota "State of the Art & Resources (2026)"
    RoPE is now the default positional encoding in virtually every frontier LLM (Llama, Mistral, Qwen, DeepSeek, Gemma). The active frontier is context extension: YaRN, LongRoPE and its 2025 successor LongRoPE2 push trained-at-4K models to 128K–1M+ tokens, and Llama 3.1's frequency-split rescaling (the `llama3` `rope_scaling` type) has become a standard config option alongside YaRN. NoPE's surprising results continue to sharpen our theoretical understanding of what position encodings actually do.
