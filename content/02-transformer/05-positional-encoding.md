...
2. **Attention temperature scaling.** As you stretch context, the softmax distribution over many more keys tends to flatten and entropy rises. YaRN multiplies the attention logits by a constant $\frac{1}{t} = 0.1 \ln(s) + 1$ (a function of the scale $s$) to *sharpen* the distribution back, recovering effective attention. Crucially this is a single scalar that can be folded into the RoPE $\cos/\sin$ magnitudes, so it costs nothing at runtime.
