...
Read the last column carefully: a *tied* model pays for this table once, an *untied* model pays twice. Llama 3 8B therefore spends about 1.05 B of its ~8 B parameters — roughly 13% — on the input embedding plus a separate LM head, while Llama 3.2 1B spends 263 M of ~1.24 B (21%) on a single shared table. Encoder-decoder models split on the same question: the original T5 (Raffel et al., 2020) ties all three — encoder input, decoder input, and the softmax output layer — while T5 v1.1 and its descendants (mT5, FLAN-T5 v1.1) set `tie_word_embeddings: false` and give the decoder its own LM head.

**Scaling law intuition.** The Kaplan et al. (2020) and Chinchilla (Hoffmann et al., 2022) scaling laws treat $d$ as one axis of model capacity. In practice, the ratio of $d$ to the number of layers and the ratio of $d$ to the number of attention heads are constrained (typically $d_{\text{head}} = d / n_{\text{heads}} = 64$ or $128$), so $d$ scales roughly as the *cube* root of the parameter count: with $N \approx 12 L d^2$ and $L$ grown alongside $d$, $N \propto d^3$. The empirical ladder agrees — GPT-2 small ($L=12$, $d=768$) to GPT-3 175B ($L=96$, $d=12{,}288$) is a roughly $2{,}000\times$ jump in non-embedding parameters for only a $16\times$ jump in $d$, an exponent of $\log 16 / \log 2000 \approx 0.36$. See [Scaling Laws: Kaplan, Chinchilla & Beyond](../03-pretraining/04-scaling-laws.html) for the full picture.
...
!!! example "Worked example: Embedding layer sizing for Llama 3 8B"
    ...
    **Logit projection FLOPs (per token, per forward pass):**
    ...
    Multiply-accumulates: $4{,}096 \times 128{,}000 = 524{,}288{,}000 \approx 0.5$ G MACs per token. Counting a MAC as 2 FLOPs (the standard $2mnk$ GEMM convention used throughout this book), that is $\approx 1.05$ GFLOP per token.

    For a batch of $B = 1$ with $T = 2048$ tokens:
    $$
    1.05 \times 2048 \approx 2{,}150 \text{ GFLOP} = 2.15 \text{ TFLOP}
    $$
    ...
    **Memory bandwidth cost during decode (single token):**
    During autoregressive decode, we generate one token at a time. The logit projection reads all $524$ M parameters from HBM to compute a single $128{,}000$-dimensional vector. At $\approx 2.0$ TB/s of HBM bandwidth (an A100 80GB), reading $\approx 1.05$ GB takes $\approx 0.53$ ms — a significant fraction of per-token latency for smaller models. This is why some inference systems use lower-precision or approximate logit computation.
...
- **Adam/AdaGrad/Adam-W** maintain per-parameter first- and second-moment accumulators. `nn.Embedding` hands the optimizer a *dense* `[V, d]` gradient that merely happens to be mostly zeros, so dense Adam still reads, writes, and steps all $V \times d$ accumulators every iteration: momentum decays, the bias-corrected step stays nonzero wherever momentum is, and AdamW's decoupled weight decay shrinks rows that were never looked up. That is wasteful in memory and bandwidth, and it keeps nudging untouched rows.
...
One subtlety with tying: `padding_idx` only masks the gradient produced by `nn.Embedding`'s own backward kernel. In an *untied* model that is the row's only gradient source, so it stays exactly zero forever. Under **tying**, the same tensor is also the LM head, and `F.linear`'s backward writes $\partial \mathcal{L} / \partial \mathbf{W}$ for *all* $V$ rows into the same `.grad` buffer — including `padding_idx`. So the pad row starts at zero (its logit is exactly $0$ at initialization) but drifts away after the first optimizer step and becomes an ordinary trained output direction. If you actually want it pinned, re-zero it explicitly (a backward hook that does `w.grad[pad_id].zero_()`, or `w.data[pad_id].zero_()` after each step). Either way, the real mechanism for excluding padding from the objective is label masking: set the label to `-100` at pad positions so `F.cross_entropy(..., ignore_index=-100)` drops them. Masking labels is mandatory; `padding_idx` is a convenience.
...
!!! interview "Interview Corner"
    ...
    **A:** Weight tying (proposed by Press & Wolf, 2017) shares the token embedding matrix $\mathbf{W}_E$ between the lookup at the input and the logit projection at the output. The motivations are: (1) it cuts the vocabulary-dependent parameter count roughly in half — for a model with $V = 128{,}000$ and $d = 4{,}096$ that saves about 524 M parameters; (2) it regularizes training by enforcing that a token's input representation is geometrically aligned with the direction the output head uses to predict that same token; (3) it generally improves perplexity at equal model size, especially for smaller models.
    ...
    The main tradeoff is that it couples two computations that might benefit from different representations — the input embedding is the start of computation, while the output head reads the fully processed hidden state. That is why tying is scale-dependent in practice: below ~4B parameters the table is 15–30% of the model and tying is nearly free capacity (Gemma, Llama 3.2 1B/3B, small Qwen, SmolLM all tie), while at 8B and above the saving is a small fraction of the model and the field generally unties (Llama 3 8B/70B, large Qwen). Encoder-decoder models show the same evolution: the original T5 shares one matrix across encoder input, decoder input, and the output softmax, whereas T5 v1.1 and later untie the LM head from that shared table.
...
!!! key "Key Takeaways"
    ...
    - Special tokens (`<pad>`, `<bos>`, `<eos>`) occupy regular vocabulary slots; using `padding_idx` in `nn.Embedding` keeps the pad row at zero with no gradient *from the lookup path* — under weight tying the LM head still writes gradient into that row, so it drifts unless you re-zero it by hand.
...
??? note "Solution"
    ...
    **Where it hurts:** if two tokens *should* be sharply distinguished as predictions but happen to sit close in embedding space, their logits interfere and the model cannot cleanly separate them — the chapter's warning that "tokens close in embedding space will interfere with each other's logit scores."
    ...
    Gradient asymmetry connects here: a token's embedding row is pulled by a *dense* gradient from the unembed path (every position contributes to every row) and only a *sparse* gradient from the input lookup (present only when that token appears). Frequent tokens get both signals; a rare token almost never appears in the input, so its row is shaped almost entirely by the unembed path, and there mostly as a small negative "not this token" push rather than as a positive target. Its "voting direction" is therefore under-trained, amplifying harmful interference precisely for the tokens that can least afford it.
