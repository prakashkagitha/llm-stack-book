...
1. *Speed.* RMSNorm roughly halves the reduction arithmetic: it accumulates only $\sum_i x_i^2$, dropping the separate mean accumulator and the centering subtraction, and it carries one fewer parameter tensor ($\boldsymbol{\beta}$). Note what it does *not* save: a memory pass. Fused LayerNorm kernels (PyTorch's `native_layer_norm` CUDA kernel, NVIDIA Apex's `FusedLayerNorm`) already read each row exactly once, computing $\mu$ and $\sigma^2$ in a single sweep with Welford's online algorithm — and a naive two-pass version can also be collapsed via $\sigma^2 = \mathbb{E}[x^2] - \mathbb{E}[x]^2$. So the win is arithmetic and parameter-count, not bandwidth, which is why measured speedups are a few percent rather than $2\times$. In wall-clock terms the savings are modest but meaningful at the million-token-per-second throughputs of large training runs.
...
At layer $l$, the residual stream has (empirically) roughly unit variance after the final norm. Because we normalize *before* the sublayer, the sublayer always sees a well-conditioned input. The output of the sublayer is added back to the (un-normalized) residual stream, whose *norm* grows slowly as $\mathcal{O}(\sqrt{l})$ in theory (its variance grows as $\mathcal{O}(l)$, since it is a sum of independent random contributions). Careful initialization (weight std $\propto 1/\sqrt{d}$, or GPT-2's $1/\sqrt{2L}$ "scaled init") makes each per-block write small, so *at initialization* the constant $c$ in front of that $\sqrt{l}$ is small and the stream barely grows. Once the model trains, the picture changes: sublayer writes become correlated, and the stream then grows *faster* than $\sqrt{l}$ — see the worked example below.
...
Some architectures apply a second normalization to the sublayer *output*, after $F$ and before the residual addition: $\mathbf{x}' = \mathbf{x} + \text{Norm}_\text{post}(F(\text{Norm}_\text{pre}(\mathbf{x})))$. This "sandwich norm" trades a little compute for extra stability by bounding what each sublayer can write into the stream. It is not exotic: **Gemma 2 and Gemma 3 ship it**, and you can see all four norms per layer in their HuggingFace modeling code and checkpoint weight names — `modeling_gemma2.py` / `modeling_gemma3.py` build a `Gemma2DecoderLayer` with `input_layernorm`, `post_attention_layernorm`, `pre_feedforward_layernorm`, and `post_feedforward_layernorm`, which appear in the state dict as `model.layers.N.pre_feedforward_layernorm.weight` and friends. (They are module names, not `config.json` fields — the config only carries `rms_norm_eps`.) A related 2024–2025 move is OLMo 2's reordered norm, which normalizes each sublayer's output rather than its input while keeping the residual path clean — reported (together with QK-norm) as a stability win over plain pre-norm at their scale.
...
!!! key "Key Takeaways"
    ...
    - RMSNorm is preferred over LayerNorm in modern models: it drops mean-centering (roughly half the reduction arithmetic, not a saved memory pass — fused LayerNorm kernels are already single-pass), removes the bias term, and achieves near-identical empirical quality while being marginally faster.
