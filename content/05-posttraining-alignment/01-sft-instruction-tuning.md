...
!!! example "Memory budget: 7B full fine-tuning vs. QLoRA"
    ...
    **Full fine-tuning (bf16 params + fp32 optimizer states):**
    ...
    - Activations (sequence length 2048, batch 4 = 8,192 tokens): ~35–45 GB without activation checkpointing, ~5–10 GB with it. (At LLaMA-7B's shapes the saved bf16 tensors run ~1.25 GB per block — the four 11008-wide MLP tensors alone are 4 × 8192 × 11008 × 2 B ≈ 0.72 GB — so 32 blocks ≈ 40 GB, plus a couple of GB of logits/softmax buffers.)
    - **Total: ~135–145 GB, or ~105–110 GB with activation checkpointing** → requires 2–4 × 80 GB A100s
...
!!! example "Computing effective batch size and tokens per second"
    ...
    Per A100 80GB, a 7B model in bf16 sustains roughly 2,500–4,000 tokens/second during forward+backward. Sanity-check that against the hardware ceiling: training costs $\approx 6N = 6 \times 7\text{e}9 = 4.2 \times 10^{10}$ FLOPs/token, and an A100 peaks at 312 TFLOP/s dense bf16, so even at an impossible 100% MFU you get $3.12\text{e}14 / 4.2\text{e}10 \approx 7{,}400$ tokens/s; 2,500–4,000 corresponds to a realistic 35–50% MFU. At 3,000 tokens/second: 5,120,000 / 3,000 ≈ 1,700 seconds ≈ 28 minutes per epoch, or ~5,100 seconds ≈ 85 minutes for all 3 epochs. (Real numbers depend heavily on sequence packing efficiency — see [Chat Templates, Data Formatting & Sequence Packing](../05-posttraining-alignment/02-chat-templates-packing.html).)
...
??? note "Solution"
    ...
    **Full fine-tuning (weights + grads + optimizer):**
    ...
    Adding a realistic ~15–25 GB of activations (with activation checkpointing) pushes the run to roughly 200 GB, so it needs $\lceil 200 / 80 \rceil = 3$ — realistically **3–4 × 80 GB A100s** once you leave headroom.
