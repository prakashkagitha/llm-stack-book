...
### The fine print: what the derivation assumes

The derivation is exact, but exactness is conditional, and the conditions are where real runs go wrong. **(1) Bradley–Terry must hold**: preferences are assumed to be generated by a *single* scalar reward, so they are transitive and consistent — an assumption human annotators routinely violate (IPO, below, is a direct response). **(2) The closed-form optimum is over the whole response space**, but the loss is only ever evaluated on the pairs in your dataset. Those pairs are usually sampled from *some other* model, so DPO is off-policy; the equivalence to RLHF degrades exactly as far as the data distribution sits from $\pi_\theta$'s own support. This is not a footnote — the Tülu 3 report and much of the 2024–2026 open-recipe literature find that preference-optimization quality is dominated by whether the pairs are **on-policy** (generated by the very model being tuned), which is also why iterative/online DPO works so well. **(3) $\pi_{\text{ref}}$ should assign non-negligible probability to the chosen responses.** If the winners came from a much stronger model, they are near-impossible strings under $\pi_{\text{ref}}$. Both log-ratios still start at exactly $0$ — the policy is initialized *from* the reference — but the chosen response's *absolute* log-probability is deeply negative, and hauling an essentially-impossible string upward is far more expensive than pushing the rejected one down. So the cheapest way to grow $\Delta$ is to crush the rejected response — dragging the chosen down with it (the degeneration failure below). The standard mitigation is the Zephyr recipe: SFT the reference on the *chosen* responses first, so the preference pairs live inside $\pi_{\text{ref}}$'s support. At small scale this effect is severe; [Post-Training: SFT, DPO, and Narrow RLVR (GRPO) That Works at 100M](../14-capstone/09-post-training.html) shows it dominating a 100M-parameter DPO run and mines on-policy pairs to fix it.
...
def dpo_training_step(policy, ref_model, batch, beta=0.1):
    ...
    batch contains, for chosen and rejected responses:
      *_input_ids (B, T), *_labels (B, T) [UNSHIFTED — same frame as input_ids;
      the shift happens below], *_loss_mask (B, T).
    """
...
!!! warning "Common pitfall: forgetting to mask the prompt and right-shift"
    ...
    Two bookkeeping bugs cause most broken DPO runs. (1) **Score only response tokens.** The `loss_mask` must zero out padding and the prompt — but be precise about *why*. In the vanilla DPO loss the prompt's log-prob is literally the same quantity in both branches (same tokens, same causal context), so it enters $\Delta$ with $+1$ and $-1$ coefficients and cancels **exactly**: leaving it in changes neither $\Delta$, nor the gradient, nor `reward_acc`. What it does corrupt is the *logged* absolute implicit rewards, and it genuinely breaks the length-normalized variants (SimPO, ORPO), where prompt tokens land in both the numerator and the per-token denominator and no longer cancel. **Padding is the dangerous case**: chosen and rejected have different lengths, so unmasked pad tokens do *not* cancel. (2) **Shift labels.** For a causal LM, `logits[:, t]` predicts token `t+1`, so labels must be the input shifted left by one — hence the `[:, 1:]` / `[:, :-1]` slicing above. Get *this* one wrong (or double-shift it by pre-shifting labels in the collator as well) and your reward accuracy hovers near 0.5 forever. Always log `reward_acc` and `reward_margin`; if accuracy isn't climbing above 0.5 in the first hundred steps, suspect the shift (or a padding-mask leak) before you touch a hyperparameter.
...
!!! example "Worked example: one DPO update by hand"
    ...
    Margin: $\Delta = 0.1\,(0.5-1.0) = -0.05$. Loss $=-\log\sigma(-0.05)=-\log(0.4875)=0.718$ nats — *above* $\log 2\approx0.693$, the loss of a coin-flip, confirming the model mis-ranks this pair. Gradient weight $=\sigma(-\Delta)=\sigma(0.05)=0.5125$ — just above the $0.5$ coin-flip mid-point (the weight only approaches its ceiling of $1$ for badly mis-ranked pairs, $\Delta\ll0$), so this example still contributes a substantial corrective gradient that raises $\log\pi_\theta(y_w)$ and lowers $\log\pi_\theta(y_l)$.
...
DPO is elegant but not perfect. Three weaknesses drive the entire variant zoo: **(1)** it overfits / pushes margins to infinity on deterministic preferences (no explicit margin cap); **(2)** it needs *paired* data $(y_w,y_l)$ for the same prompt, which is expensive; and **(3)** it needs a frozen reference model, doubling the *weight* memory and adding a forward pass. Each variant attacks one or more of these.
...
A cheaper fix for the *noise* half of that problem — one that never leaves the DPO loss — is **conservative DPO (cDPO)**, a.k.a. DPO with label smoothing: assume each preference label is flipped with probability $\epsilon$ and smooth the binary cross-entropy target accordingly,

$$
\mathcal{L}_{\text{cDPO}} = -(1-\epsilon)\log\sigma(\Delta) \;-\; \epsilon\log\sigma(-\Delta).
$$

Setting the derivative to zero gives $\sigma(\Delta)=1-\epsilon$, i.e. a **finite** optimal margin $\Delta^\star=\log\frac{1-\epsilon}{\epsilon}$ (with $\epsilon=0.1$, $\Delta^\star\approx 2.2$) instead of $+\infty$ — so a mislabeled pair can no longer drag the margin without bound. In TRL this is one argument: `DPOConfig(label_smoothing=0.1)`. Choose $\epsilon$ to match your estimated annotator error rate; it should be $0$ for programmatically verified labels and clearly positive for crowd-sourced ones.
...
def orpo_loss(pol_chosen_logps, pol_rejected_logps,
    ...
    # term collapses to a plain (summed_chosen - summed_rejected) log-prob difference.
...
**SimPO** (Meng, Xia, Chen, *SimPO: Simple Preference Optimization with a Reference-Free Reward*) keeps DPO's contrastive log-sigmoid shape but makes two changes. First, drop the reference model: define the reward as the **length-normalized average log-probability**, $r(x,y)=\tfrac{\beta}{|y|}\log\pi_\theta(y\mid x)$. Length normalization is the key insight. DPO's implicit reward is a *sum* of per-token log-ratios, so it is **unnormalized**: its magnitude grows with the number of tokens, while generation ranks candidates by *average* per-token log-likelihood. SimPO's argument is that this mismatch between the training reward and the generation metric is what lets DPO exploit length: once the chosen response's per-token log-ratio has been pushed positive, every additional token *adds* reward, so the reward correlates positively with length and the policy learns to pad — a pull the data reinforces, since chosen responses in preference data are typically longer than rejected ones. SimPO's own ablation makes the point: drop the $1/|y|$ factor and the learned reward becomes strongly length-correlated, producing markedly longer, lower-quality generations. Normalizing by $|y|$ puts the reward on the same scale generation actually uses and breaks that correlation. Second, add a **target reward margin** $\gamma$ that the chosen must beat the rejected by:
...
The practitioner's decision tree: *paired data + can afford a reference* → start with **DPO** ($\beta=0.1$). *Noisy or near-deterministic labels* → **IPO** or cDPO. *Only unpaired ratings* → **KTO**. *Want one stage / no reference model / tight memory* → **ORPO** or **SimPO**. There is no universal winner; results are dataset- and base-model-dependent, and you should treat the choice as a hyperparameter to sweep.

### Running the whole family in TRL

You now know every loss in the table from the inside, which is exactly what you need to use the production implementation intelligently. **HuggingFace TRL** is the reference open-source library for all of them, and its DPO family takes preference datasets with `prompt` / `chosen` / `rejected` columns (either plain strings or chat-message lists, which TRL renders with the tokenizer's chat template):
...
MODEL = "Qwen/Qwen2.5-0.5B-Instruct"          # your SFT checkpoint in a real run
policy    = AutoModelForCausalLM.from_pretrained(MODEL, torch_dtype="bfloat16")
tokenizer = AutoTokenizer.from_pretrained(MODEL)
pref_ds   = load_dataset("HuggingFaceH4/ultrafeedback_binarized", split="train_prefs")

cfg = DPOConfig(
    output_dir="dpo-out",
    beta=0.1,                       # the KL temperature β from the derivation
    loss_type="sigmoid",            # "sigmoid" = vanilla DPO; "ipo" = the IPO squared loss
    label_smoothing=0.0,            # >0 → conservative DPO (cDPO) for noisy labels
    precompute_ref_log_probs=True,  # cache log π_ref once, then free the reference model
    learning_rate=5e-7,             # DPO LRs are 10–100× smaller than SFT LRs
    num_train_epochs=1,
    per_device_train_batch_size=2,
    gradient_accumulation_steps=16, # effective batch ≈ 32–128 pairs is typical
    max_length=1024, max_prompt_length=512,
    bf16=True,
)

trainer = DPOTrainer(
    model=policy,          # an AutoModelForCausalLM, optionally PEFT-wrapped
    ref_model=None,        # None + a LoRA policy → TRL uses the adapter-disabled base as π_ref
    args=cfg,
...
Two implementation details worth knowing, because they are exactly the optimizations our from-scratch code left on the table: TRL runs a **concatenated forward** — chosen and rejected are packed into one batch so each model sees a single forward per step instead of two — and `precompute_ref_log_probs` implements the caching trick described above. The rest of the family maps onto sibling trainers: **KTO** → `KTOTrainer` / `KTOConfig` on an unpaired dataset (`prompt`, `completion`, `label`), with `desirable_weight` / `undesirable_weight` playing the roles of $\lambda_D,\lambda_U$; **ORPO** → `ORPOTrainer` / `ORPOConfig`, whose `beta` is the odds-ratio weight $\lambda$ (there is no reference model to temper); **CPO and SimPO** → both live in `CPOTrainer`, with `CPOConfig(loss_type="simpo", simpo_gamma=…)` selecting the length-normalized SimPO reward and `cpo_alpha` controlling the SFT anchor (set it to `0.0` for pure SimPO). Online/iterative DPO — regenerate pairs from the current policy, label them with a reward model or an LLM judge, re-run the loss — is `OnlineDPOTrainer`. Check the trainer docs for the version you install; TRL's `loss_type` menu grows every release. Beyond TRL, the same losses ship in **OpenRLHF**, **Axolotl**, **LLaMA-Factory**, and HuggingFace's **alignment-handbook** (the Zephyr recipe); the mechanics you just implemented are identical in all of them. See [TRL: HuggingFace's RL Library](../06-rl-infra/03-trl.html) for the library end to end.
...
!!! key "Key Takeaways"
    ...
    - **The derivation's fine print:** it assumes Bradley–Terry preferences and, in practice, breaks down as the pairs drift off-policy or outside $\pi_{\text{ref}}$'s support — which is why on-policy pair generation (and SFT-on-the-chosen before DPO) matters more than the choice of variant.
    - **IPO** caps the margin (squared error to $\tfrac{1}{2\beta}$, noise-robust); **cDPO** does the cheap version via label smoothing $\epsilon$ (finite optimum $\log\frac{1-\epsilon}{\epsilon}$); **KTO** drops pairing (prospect-theory value on unpaired thumbs up/down); **ORPO** drops the reference (SFT + odds-ratio, single stage); **SimPO** drops the reference and the length bias (length-normalized reward + margin $\gamma$); **CPO** is reference-free contrastive + SFT anchor.
...
**4.** (Quantitative) SimPO argues that an *unnormalized* (summed) log-prob reward is on the wrong scale, because it grows with token count while generation ranks candidates by *average* per-token log-likelihood. Consider two responses to the same prompt that are equally fluent per token — both have an average log-prob of $-1.0$ per token — but response $A$ has length $10$ (summed log-prob $-10$) and response $B$ has length $30$ (summed log-prob $-30$). (a) Using the **reference-free summed** reward $r=\beta\sum_t\log\pi_\theta$ with $\beta=0.1$ (this is CPO's reward, and SimPO's reward with the $1/|y|$ factor removed), compute $r_A$ and $r_B$ and say which the summed reward prefers. (b) Using SimPO's length-normalized reward $r=\frac{\beta}{|y|}\log\pi_\theta$ with $\beta=2.0$, compute $r_A$ and $r_B$. (c) In one sentence, state what this shows about a length-blind summed reward.
...
??? note "Solution"
    ...
    (c) A summed reward conflates *quality* with *length*: the score accumulates one term per token, so it moves with token count independent of per-token quality — here, where every term is negative, the longer response looks worse purely for being longer. (The sign of that drift depends on the sign of the per-token term: for DPO's summed log-*ratio*, per-token terms turn positive once the chosen response is pushed up, so the reward instead *grows* with length — which is the length exploitation SimPO documents.) Either way the reward is not on the scale generation optimizes; dividing by $|y|$ puts it back on the per-token scale and removes the length dependence.
