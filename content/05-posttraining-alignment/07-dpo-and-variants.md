...
**SimPO** (Meng, Xia, Chen, *SimPO: Simple Preference Optimization with a Reference-Free Reward*) keeps DPO's contrastive log-sigmoid shape but makes two changes. First, drop the reference model: define the reward as the **length-normalized average log-probability**, $r(x,y)=\tfrac{\beta}{|y|}\log\pi_\theta(y\mid x)$. Length normalization is the key insight. DPO's *summed* log-prob is length-sensitive — every extra token adds another negative term, so the summed reward mechanically favors *shorter* sequences — while generation ranks candidates by *average* per-token log-likelihood. SimPO's argument is that this mismatch between the training reward and the generation metric is what lets DPO exploit length (chosen responses in preference data are typically longer than rejected ones, and the per-token contrastive push then systematically inflates response length); normalizing by $|y|$ puts the reward on the same scale generation actually uses. Second, add a **target reward margin** $\gamma$ that the chosen must beat the rejected by:
