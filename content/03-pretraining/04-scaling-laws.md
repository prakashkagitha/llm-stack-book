...
The cleanest and most useful parameterization — popularized by Hoffmann et al. (the Chinchilla paper), following the three-term additive form of Rosenfeld et al. (2020) — treats $N$ and $D$ jointly and adds the irreducible floor:
...
!!! note "Why a power law and not something else?"
    There is no single agreed first-principles derivation, but the leading intuition comes from data manifolds. If the data lives on a manifold of intrinsic dimension $d$, and a model of size $N$ effectively tiles that manifold with $\sim N$ pieces, then each piece has linear size $s \sim N^{-1/d}$. A piecewise-linear (ReLU) approximant is off by $\sim s^2$ pointwise on a patch that size, so the loss scales like $s^4 = N^{-4/d}$ — a power law whose exponent $\alpha \approx 4/d$ is set by the *intrinsic dimensionality of language*, not by the architecture. Sharma & Kaplan formalized this argument and derived exactly $\alpha \approx 4/d$ for both squared-error and cross-entropy losses (read backwards, a measured $\alpha \approx 0.34$ implies $d \approx 12$). The practical upshot: the exponents are remarkably architecture-insensitive. Swap GELU for SwiGLU, add RoPE, change the aspect ratio within reason — the *slope* barely moves, even though the *offset* (the constant $A$) does.
...
Their answer, derived by fitting $L(N, D)$ and minimizing under the constraint $C = 6ND$, was that **most of the budget should go into model size**. As compute grows, you should grow $N$ fast and $D$ slowly. Their fitted relationship was roughly $N_{\text{opt}} \propto C^{0.73}$ and $D_{\text{opt}} \propto C^{0.27}$ — model size soaking up nearly three-quarters of every additional decade of compute. This is why the models of 2020–2021 (GPT-3 at 175B params trained on ~300B tokens, Gopher at 280B on ~300B tokens, MT-NLG at 530B on ~270B tokens) were **enormous but relatively data-starved**: roughly 0.5–2 tokens per parameter.

### The bug: a confounded learning-rate schedule

Two years later, Hoffmann et al. (the **Chinchilla** paper, *Training Compute-Optimal Large Language Models*) re-ran the analysis far more carefully and reached a dramatically different conclusion. Chinchilla's own diagnosis of where Kaplan went wrong: **the learning-rate schedule was not matched to the token budget**.

Recall from [Learning Rate Schedules, Warmup, Batch Size & Hyperparameters](../03-pretraining/10-lr-schedules-hparams.html) that a cosine schedule should decay to its minimum *exactly at the end of training*. Kaplan's experiments largely reused a single long schedule and read off intermediate losses — so the shorter runs (fewer tokens) were evaluated mid-decay, with their learning rate still too high, making them look worse than they truly were. The argument is that this systematically **understated the value of data**, biasing the optimal allocation toward parameters. Chinchilla retrained every point with a schedule decayed to its own endpoint and used three independent estimation methods that all agreed.

!!! note "What later replications found: it was mostly the parameter counting"
    Treat "it was the cosine schedule" as Chinchilla's *hypothesis*, not a settled fact — two 2024 studies re-ran the comparison end to end and decomposed the gap. Pearce & Song (*Reconciling Kaplan and Chinchilla Scaling Laws*, 2024) attribute most of it to Kaplan fitting and reporting $N$ as **non-embedding** parameters while Chinchilla used **total** parameters: at Kaplan's small scales the embedding and output matmuls are a large fraction of the model — exactly the effect the FLOP-counter section above measures — and that accounting choice alone rotates the fitted allocation exponent from $\approx 0.73$ toward $\approx 0.5$. Porian et al. (*Resolving Discrepancies in Compute-Optimal Scaling of Language Models*, NeurIPS 2024) reach Chinchilla's exponents by correcting three factors — last-layer compute, warmup duration, and scale-dependent optimizer tuning — and explicitly report that careful LR decay was *not* essential. The methodological lesson survives intact and gets *broader*: what biases a scaling law is any quantity that is mis-measured differently at different scales, whether that is the schedule, the warmup, or what you count as a parameter.
...
This is exactly why **Llama-style** models are trained on token counts vastly exceeding 20× their parameter count. Llama-2-7B saw ~2T tokens (~280 tokens/param); Llama-3-8B saw ~15T tokens (~1900 tokens/param) — roughly **two orders of magnitude past Chinchilla-optimal** — and by 2025 the ceiling had climbed higher still, with models like Qwen3 pretrained on roughly 36T tokens. These models are *worse than Chinchilla-optimal for their training FLOPs* but *far better per inference FLOP*, which is what their deployers actually pay for.
...
2. **Reproduce the Kaplan bias.** Deliberately include under-converged runs. A single cosine schedule tuned for the *longest* run leaves every **shorter** run read off mid-decay with its LR still too high, so it is the low-$D$ rows that come out inflated. Multiply the loss of every run with $D \leq 10^{9}$ by 1.03, refit, and report $\alpha$, $\beta$ and the allocation exponent $\beta/(\alpha+\beta)$ against the clean fit; then repeat with the corruption applied to every run with $D \leq 10^{10}$ instead. You should find that a 3% systematic error on one end of the grid can move the allocation exponent by several hundredths — comparable to the bootstrap spread computed above — and, more unsettling, that the *sign* of the shift depends on where you put the cutoff. Explain in one sentence why that makes "exclude under-converged runs" a hard requirement rather than a nicety: a systematic error the fit cannot see is not something you can correct for after the fact.
...
!!! sota "State of the Art & Resources (2026)"
    ...
    **Recent advances (2023–2026)**
    ...
    - [Porian et al., *Resolving Discrepancies in Compute-Optimal Scaling of Language Models* (NeurIPS 2024)](https://arxiv.org/abs/2406.19146) — recovers Chinchilla's exponents from Kaplan's setup by fixing three factors (last-layer compute, warmup duration, scale-dependent optimizer tuning), and finds careful LR decay was *not* the essential ingredient.
    - [Pearce & Song, *Reconciling Kaplan and Chinchilla Scaling Laws* (2024)](https://arxiv.org/abs/2406.12907) — attributes most of the remaining gap to Kaplan counting non-embedding rather than total parameters at small scale.
    - [Kumar et al., *Scaling Laws for Precision* (2024)](https://arxiv.org/abs/2411.04330) — extends the scaling law with a precision axis, predicting loss under low-precision (fp8/fp4-style) training and inference; increasingly relevant as frontier runs move below bf16.
...
- Rosenfeld, Rosenfeld, Belinkov, Shavit. *A Constructive Prediction of the Generalization Error Across Scales*. ICLR 2020. (The additive $a\,m^{-\alpha} + b\,n^{-\beta} + c_\infty$ form that Chinchilla later fitted.)
