...
For speed at billion-document scale, fastText classifiers (which average *learned* embeddings of hashed word and character $n$-grams — there is no IDF weighting anywhere in the model) and plain linear classifiers over TF-IDF or hashed bag-of-words features are common. They run in microseconds per document, enabling an entire CommonCrawl WET dump (on the order of 3 billion URLs, roughly 9 TB of compressed WET text) to be classified in hours on a single machine.

Be precise about which recipe is which, because the two are often conflated. **CCNet** does not train a discriminative classifier at all: it fits a *per-language KenLM 5-gram model on Wikipedia* and bins each document by perplexity into head/middle/tail thirds, keeping the head (and optionally the middle). The **Wikipedia-vs-crawl discriminative classifier** — positives from WebText/Wikipedia/Books, negatives from raw crawl — is the GPT-3 recipe. Be precise about GPT-3's own implementation too, which is routinely misremembered as fastText with a top-$k$% cut: Brown et al. (2020, Appendix A) trained a *logistic regression* classifier over features from Spark's standard tokenizer and `HashingTF`, and then retained documents with a deliberately *noisy* rule, `np.random.pareto(alpha) > 1 - document_score` with $\alpha = 9$, rather than a hard percentile threshold. The Pareto noise keeps mostly high-scoring documents but lets some low-scoring ones through on purpose, preserving diversity; a hard top-percentile cut (what the code below implements, and what most reimplementations use) is a stricter, simpler variant. Which positives you choose matters more than the model class: DCLM (Li et al., 2024) found that a plain fastText classifier trained with *instruction-style* positives (OpenHermes-2.5 plus highly-upvoted ELI5 posts) rather than Wikipedia produced a markedly stronger filter, and that filter is what defines DCLM-baseline. The ROOTS corpus (used for BLOOM) went in another direction, adding domain experts to annotate quality labels.

Here is a compact but complete trainable classifier: Wikipedia paragraphs are the positive class, raw CommonCrawl paragraphs the negative class, and we keep the top percentile by predicted quality on a held-out split (a GPT-3-style discriminative filter, not CCNet's perplexity binning — with the hard threshold standing in for GPT-3's Pareto-noised acceptance rule). It uses `HashingVectorizer` so there is no vocabulary to store -- fixed memory regardless of corpus size -- with a `LogisticRegression` head; the commented block shows the faster `fasttext.train_supervised` alternative used at billion-document scale.
...
!!! example "Worked example: deduplication at CommonCrawl scale"
    Consider processing one CommonCrawl snapshot. Start from the size anchor used throughout this chapter: ~9 TB of *compressed* WET text, which unpacks at gzip's usual ~3.5x on plain text to roughly 30 TB of characters, or on the order of 8 trillion tokens at ~4 bytes per token. So:

    - Raw WET files: approximately 8 trillion tokens before filtering.
    - After language ID (keep English, threshold 0.65): approximately 3.5 trillion tokens (English is a little under half the crawl).
    - After heuristic filters: approximately 1.2 trillion tokens (the 50–70% cut quoted in the Key Takeaways).
    - After exact dedup (SHA-256 on normalized text): approximately 1.1 trillion tokens (removes ~10% exact duplicates — mostly mirror sites and syndicated content).
    - After MinHash fuzzy dedup (threshold 0.8, 128 hashes, 16 bands): approximately 800 billion tokens (removes another ~25% near-duplicates).
    - After benchmark decontamination (13-gram, against 20 standard benchmarks): approximately 798 billion tokens (removes ~0.3% but dramatically improves eval integrity).

    Final yield: roughly 10% of raw tokens, with substantially higher diversity, lower memorization risk, and honest benchmark evaluations. The yield ratio will vary — older crawls (2014–2018) tend to have more boilerplate; newer crawls have more unique content.

    Sanity-check the order of magnitude against a published pipeline. This cascade has *no* model-based quality classifier, which is where modern pipelines shed most of what is left: FineWeb (which also re-extracts from WARC with trafilatura instead of using WET, discarding boilerplate the WET files keep) produced 15T tokens from 96 dumps, about 150 billion per dump — another ~5x below the number above. If your funnel lands at 10% of raw and you then add a DCLM/FineWeb-Edu-style classifier at `keep_frac = 0.20-0.30`, you should expect to end near 2-3% of raw. A funnel that reports 30% survival after "full" filtering is almost certainly under-filtering.
