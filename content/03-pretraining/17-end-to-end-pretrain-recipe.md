...
**Three small patches to `train.py` first.** 3.5's script exposes the data, parallelism, batch, schedule and checkpoint flags — but *not* the model shape. It builds the model as `model = GPT(ctx=args.ctx)`, which pins `d=768, h=12, n_layers=12` to the constructor's defaults, so out of the box every command below would train the same 124M GPT-2-small no matter which row of the Stage-3 table you were aiming at. Add the three missing flags — the same three `eval_ppl.py`, `export_hf.py` and `sample.py` already take, so the whole chapter agrees on how a config is spelled — and, per Stage 3, apply the GPT-2 initialization to the constructed model. The third patch is bf16: 3.5's script gets bf16 *only* on the `fsdp` branch, from FSDP's `MixedPrecision` config, so the `--parallel ddp` commands below would otherwise run in fp32. That is harmless for the CPU toy, but it leaves the single-GPU 124M run far short of the bf16 tensor-core path that every MFU, tokens/s and wall-clock figure in this chapter is computed against.
...
# ... and third: bf16 on the ddp path. FSDP already computes in bf16 via its
# MixedPrecision config, so autocast is only needed (and only enabled) when the
# ddp branch is running on CUDA; on CPU/gloo it degrades to a no-op.
amp_on = args.parallel == "ddp" and device.type == "cuda"
# then, inside the micro-batch loop of the per-step body above:
#     with sync:
#         with torch.autocast(device_type=device.type, dtype=torch.bfloat16,
#                             enabled=amp_on):
#             loss = nn.functional.cross_entropy(model(x).view(-1, 50304),
#                                                y.view(-1)) / args.grad_accum
#         loss.backward()      # backward stays OUTSIDE the autocast context
```
...
`--parallel {ddp,fsdp}` is the entire switch between "replicate the model" and "shard it." Under `fsdp`, `ShardingStrategy.FULL_SHARD` gives you ZeRO-3 semantics (parameters, gradients, and optimizer state all sharded across the group), and bf16 arrives via FSDP's `MixedPrecision` config rather than a separate autocast call (the `ddp` branch has no such config, which is exactly why it needs the autocast patch above) — see [Mixed Precision, bf16 & FP8 Training](../03-pretraining/08-mixed-precision-fp8.html) for why bf16, not fp16, is the default here (its wider dynamic range avoids the overflow failure mode in Stage 7 below).

**`torch.compile`, the one flag not in the launch line.** The MFU figures quoted below assume the model has been compiled. Adding it is one statement — `model = torch.compile(model)` on the raw `GPT` *before* the DDP/FSDP wrap — and it typically buys a substantial throughput improvement at these scales by fusing the pointwise chains around LayerNorm, GELU and the residual adds into far fewer kernel launches. Three things to know: the first step pays a one-time compilation cost of tens of seconds (do not include it in your ms/step average); every distinct input shape triggers a fresh compile, so keep `local_bsz` and `ctx` fixed for the whole run (a ragged last batch will silently recompile); and compilation composes with FSDP only when the wrapper is constructed with `use_orig_params=True`, which 3.5's `train.py` already does. Two practical notes. `torch.compile` returns an `OptimizedModule` that holds the real model as `_orig_mod`, so its `state_dict()` keys all gain an `_orig_mod.` prefix — save `model._orig_mod.state_dict()` (or strip the prefix on load), or `eval_ppl.py` and `export_hf.py` will reject the checkpoint on a key mismatch. And if a compile error blocks you, plain `torch.compile(model)` is already the most permissive setting (`fullgraph=False` and `mode=None` are the defaults, and Dynamo falls back to eager at graph breaks on its own); the escape hatches that actually change behaviour are `torch._dynamo.config.suppress_errors = True`, which downgrades a compile failure to a warning and runs that region eagerly, and decorating the offending submodule's `forward` with `@torch._dynamo.disable` so it is never traced. The mechanics — Dynamo's graph capture, Inductor's fusion and Triton codegen, CUDA graphs — are [Kernel Fusion, torch.compile, CUDA Graphs & Compilers](../04-kernels-efficiency/09-compilers-fusion.html).
...
A first real run rarely goes perfectly. Here are the failure signatures you will actually see, roughly in the order you're likely to hit them:

| Symptom | Likely cause | Fix |
|---|---|---|
| Loss stuck near $\ln(\text{vocab})$ | targets scrambled relative to inputs, LR far too low, corrupt/repeated-token shard | decode a batch to real text; verify `x = w[:-1], y = w[1:]`; raise LR |
| Loss collapses far *below* $\ln(\text{vocab})$ in tens of steps | `y` equals `x` (the `y = w[:-1]` typo) — a causal model solves that by copying | verify the shift; check the loss is over all `vocab` classes |
...
`ppl = exp(mean_ce)` is the standard definition — the effective branching factor of the model's next-token distribution. Because perplexity is tokenizer-dependent (a coarser vocabulary reports a lower perplexity for the same underlying predictive power), the tokenizer-agnostic **bits-per-byte** normalization is the fairer number for cross-tokenizer comparisons; its conversion from nats/token is covered in [The Pretraining Objective & Loss](../03-pretraining/03-pretraining-objective.html). Expected perplexity, reading straight off the scale table at the end of this chapter: on the order of **~20-35** for the TinyStories toy after its 2000-step run, **~22** for the single-GPU 124M model, and **~14** for the 8-GPU 1.3B model — lower is better, and the trend across the two FineWeb-Edu rows (bigger model + more tokens = lower perplexity) is the entire content of scaling laws made concrete. Do not read the toy's number against the other two: it is measured on a different and far easier corpus, and it is high only because 4M tokens is a fraction of a fraction of what that model could absorb — train the same toy for 10x longer and it drops into the low teens.
...
What to expect, per scale: the **TinyStories toy** (2000 steps on ~4M tokens, per the Stage-5 launch line) produces recognizably children's-story *vocabulary* and mostly well-formed short clauses — "Once upon a time there was a little girl who..." continues in the right register — because that is exactly the narrow distribution it was trained on. Do not expect sentences to stay on topic, let alone a story to hold together: 4M tokens against ~18M parameters is far below any compute-optimal budget, and at a val loss around 3 nats/token the model has learned the corpus's local statistics and little else. If you want the toy to actually finish a coherent story, raise `--steps` (10k steps is ~20M tokens and still an afternoon; the published TinyStories models saw hundreds of millions). The **124M single-GPU model**, trained on a few billion tokens of FineWeb-Edu, produces text that is locally fluent — grammatical clauses, plausible next words, topically consistent for a sentence or two — but wanders globally, losing the thread of an argument or drifting topic over a paragraph; this is the classic signature of a model with enough capacity and data to nail local statistics but not yet enough to hold long-range coherence.
...
|---|---|---|---|---|---|---|---|---|
| Laptop / CPU toy | 1 laptop CPU (or MPS / 1 small GPU) | n_layer=6, d_model=256, n_head=8, ctx=256, TinyStories | ~18M | ~4M (a small slice of one epoch) | ~30-90 min CPU; ~5-15 min MPS/GPU | loss ~3.0-3.5 / ppl ~20-35 after 2000 steps (a 10k-step run gets to roughly ~2.5 / ~12; the ~1.3 nats of a fully-trained TinyStories model needs hundreds of millions of tokens) | not meaningful on CPU | `torchrun --nproc_per_node=1 train.py --data ./data --parallel ddp --backend gloo --ctx 256` |
