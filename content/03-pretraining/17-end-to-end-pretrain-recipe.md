...
The model is not reprinted here. It **is** the GPT built module-by-module in [Building a GPT From Scratch](../02-transformer/07-build-gpt-from-scratch.html) — token embedding, learned positional embedding, a stack of pre-norm causal-attention + MLP blocks, a final LayerNorm, and a weight-tied `lm_head` — and it is the exact compact `GPT` class embedded inside chapter 3.5's `train.py` (constructor `GPT(vocab, d, h, n_layers, ctx)`, mirroring `GPTConfig`'s `vocab_size`, `n_embd`, `n_head`, `n_layer`, `block_size` fields under shorter names so the whole training script fits on one screen). We do not reprint the modules; we only fix the numbers.
...
The optimizer is **AdamW**, `betas=(0.9, 0.95)`, `weight_decay=0.1`, with the standard split that excludes 1-D parameters (biases, LayerNorm/RMSNorm gains) from weight decay — only 2-D weight matrices get decayed, because decaying a norm gain toward zero destabilizes the normalization it controls. This is exactly `configure_optimizer` from [Building a GPT From Scratch](../02-transformer/07-build-gpt-from-scratch.html), and the *why* behind decoupled decay (as opposed to L2-regularization folded into the gradient) is derived in [Optimizers: SGD, Adam, Adafactor, Lion, Muon & Shampoo](../03-pretraining/09-optimizers.html).
