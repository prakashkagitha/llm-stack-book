...
When you provide $n$ examples $(x_1, y_1), \ldots, (x_n, y_n)$, the model's next-token distribution conditions on all of them. The examples effectively shift the model's prior over output space without any weight update. This is why few-shot largely survives *randomly replaced* labels — each demonstration's gold label swapped for one drawn uniformly from the label set, so the input→label mapping is destroyed rather than merely relabelled (Min et al., 2022): the demonstrations mostly teach format, label space, and input distribution rather than the input→label mapping, which pretraining already supplied.
...
**1.** (Conceptual) Section 8.4 states that few-shot learning "largely survives *randomly replaced* labels," because the demonstrations "mostly teach format, label space, and input distribution rather than the input→label mapping." Explain the mechanism this implies, and use it to justify two of the four best practices in the "Example Selection and Ordering" list. Then state how the scale-dependence result of Wei et al. (2023) qualifies the conclusion. If labels carried no information at all, would you expect few-shot to ever beat zero-shot? Explain.

??? note "Solution"
    The finding (Min et al., 2022, referenced in §8.4 and the SOTA box) says that replacing the *label* attached to each demonstration with one sampled at random from the label set barely hurts accuracy on many tasks. The mechanistic reading given in the chapter: in-context learning is not standard supervised learning over the demonstration labels. Instead the demonstrations communicate the *format*, *register*, *granularity*, and *implicit rubric* of a valid output, and they shift the model's prior over the output space without any weight update. The label slot mainly teaches the model *what a label looks like and where it goes*, not the true input->label mapping — the true mapping was already learned in pretraining.
    ...
    The scale-dependence qualifier: the "labels don't matter" headline is not a law, it is a statement about a particular model-size regime. Wei et al. (2023) ran the sharper *systematically flipped* protocol — a consistent but wrong mapping rather than i.i.d. random labels — and found that small models ignore the flip and keep their pretrained priors, while large models increasingly override those priors and follow the in-context mapping. So the mechanism above describes the small/mid-scale end of the curve; at frontier scale demonstrations genuinely do teach the mapping, and wrong labels hurt. Practically: never take label correctness as optional just because Min et al. found it cheap on a 2022-era model.
