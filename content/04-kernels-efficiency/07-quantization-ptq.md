...
!!! tip "Practitioner tip"
    ...
    Which activation axis can you actually hang a scale on? The matmul $Y_{t,j} = \sum_c X_{t,c} W_{c,j}$ contracts over the *channel/feature* axis $c$, so a per-**channel** activation scale $s_c$ sits *inside* the summation and cannot be factored out of an integer GEMM at all — this is the point the SmoothQuant paper makes when it says per-channel activation quantization "cannot be realized with hardware-accelerated GEMM kernels". A per-**token** scale $s_t$, on the other hand, is constant across the contraction and factors straight out: $Y_{t,j} \approx s_t\, s^W_j \sum_c q^{X}_{t,c} q^{W}_{c,j}$. So the legal axes are the *non-contracted* ones: per-token (or per-tensor) for activations, per-output-channel for weights. That per-token scale has to be recomputed every forward pass because the token count varies, but that is just a cheap per-row max-reduction — "dynamic per-token" activation quantization is the standard W8A8 recipe in `bitsandbytes`, vLLM and `llm-compressor`, and it is exactly what `llm_int8_matmul` does later in this chapter (`sx = Xr.abs().amax(dim=1, ...)`). SmoothQuant, below, exists precisely *because* the per-channel activation scale you would like to use is illegal inside the GEMM: it migrates that per-channel difficulty into the weights, where the same axis is a non-contracted one.
