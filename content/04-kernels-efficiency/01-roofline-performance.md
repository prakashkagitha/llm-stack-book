...
Analytic FLOP formulas are for planning; for a *measured* MFU you want the FLOPs your graph actually dispatched. PyTorch ships this: `torch.utils.flop_counter.FlopCounterMode` is a `TorchDispatchMode` that intercepts every op with a registered FLOP formula (matmuls, `scaled_dot_product_attention`, convolutions) and tallies them — including the backward pass. One convention it does *not* infer for you is causal masking: `sdpa_flop` charges the full $s\times s$ score matmuls regardless of `is_causal` (the source comment reads `# NB: We aren't accounting for causal attention here`), so for a causal LM you must halve the SDPA contribution yourself if your analytic estimate used the causal convention — otherwise the audit over-counts attention by exactly the factor of two flagged earlier. Pair it with `triton.testing.do_bench`, the microbenchmark helper that ships with PyTorch's Triton dependency and handles warmup, L2-cache flushing between reps, and synchronization for you.
...
??? note "Solution"
    ...
    **(b)** $I = \dfrac{1.72\times10^{10}}{1.38\times10^{8}} \approx 125\ \text{FLOP/B}$. Since $125 < 156$, it is **memory-bound** on the A100 — and this counts only the $QK^\top$ write; a faithful naive attention also *reads* $S$ back for the softmax, writes the probabilities $P$, and reads $P$ again for $PV$, roughly *quadrupling* the $s^2$ traffic to $\approx 5.4\times10^{8}$ B while only doubling the FLOPs (the $PV$ matmul adds another $2s^2 d_h$, for $\approx 3.4\times10^{10}$ total). That gives $I \approx 63$ FLOP/B — still well below the A100's 156, so real materialized attention is decisively memory-bound. The huge $s\times s$ intermediate is the culprit: its byte cost scales as $s^2$ while the useful reuse does not.
