...
After the last block, divide once: $O \leftarrow O / \ell$. The recurrence is *mathematically exact* — it computes the same softmax-then-matmul, unlike an approximation such as linear attention — though not bit-identical in floating point, since the repeated $\alpha$ rescales change the rounding and accumulation order. And no $N\times N$ tensor ever touched HBM. The kernel below implements the forward pass for one `(batch, head)` per program row-block.
...
**The memory win, quantified.** The naive path reads/writes the $N\times N$ score matrix. Our kernel keeps $Q$, the running $O$, $m$, and $\ell$ in registers/SRAM and streams $K$,$V$ tiles, so no $N\times N$ tensor is ever *materialized*: the memory footprint per head drops from $O(N^2)$ to $O(Nd)$. HBM *traffic* drops too, but not to linear — each of the $N/\texttt{BLOCK\_M}$ query-block programs streams all of $K$ and $V$, so the kernel moves $\Theta(N^2 d/\texttt{BLOCK\_M}) = \Theta(N^2 d^2/M)$ elements ($M$ = SRAM size), i.e. smaller than the naive path's $\Theta(N^2)$ score-matrix round-trips by the tile factor $\Theta(M/d^2)$, which is the IO bound derived in [FlashAttention I](../04-kernels-efficiency/02-flash-attention-1.html). Linear *memory* plus roughly an order of magnitude less *traffic* is *the* reason long-context attention is feasible at all. The backward pass needs the same statistics recomputed (FlashAttention recomputes $S$ in the backward rather than storing it — trading FLOPs for memory), which we implement in Triton as Kernel 5 below (the forward above already saved the per-row logsumexp L that the backward needs).
...
!!! note "The atomics-free alternative"
    The way to avoid `tl.atomic_add` on `dQ` is to split the backward into **two phases with disjoint ownership**: the kv-parallel `dK`/`dV` phase above, plus a q-parallel `dQ` phase that loops over key blocks for a fixed query block, exactly mirroring the forward. Each program then owns its output exclusively and there's no contention. The official Triton `06-fused-attention.py` tutorial does exactly this, running both phases inside a single `_attn_bwd` launch (`_attn_bwd_dkdv` and `_attn_bwd_dq` are `@triton.jit` *device helpers* called from it, not separate launches); issuing them as two kernels works equally well. Not everyone avoids atomics, though: Dao-AILab/flash-attention's CUDA backward accumulates `dQ` with `atomicAdd` into a separate fp32 `dq_accum` buffer and converts it to the output dtype in a follow-up kernel — the same strategy as our `tl.atomic_add`, just with the accumulator managed explicitly. Both are production-proven; the exclusive-ownership split is simply the atomics-free option. This is the same forward-parallel-over-queries / backward-parallel-over-keys asymmetry discussed in [FlashAttention 2 & 3](../04-kernels-efficiency/03-flash-attention-2-3.html); we used one kernel plus `tl.atomic_add` above because it's shorter to read end-to-end.
...
Run the same test with `q, k, v` in fp32 and you should see errors tighten to roughly `1e-3` — the fp16 case's larger tolerance is accumulation-order noise, not a bug. Two failure signatures worth memorizing so you recognize them instantly: **dropping the $D_i$ subtraction** (using `dS = P * dP * sm_scale` instead of `P * (dP - D_i) * sm_scale`) leaves the softmax Jacobian's rank-1 correction term out, so row $i$ of `dQ` picks up a spurious $\text{sm\_scale}\cdot D_i\,(P_i K)$ — a per-row *scalar* times the attention-weighted key vector, not an additive constant — and `dK` is likewise off by $\text{sm\_scale}\,(D \odot P)^\top Q$, while `dV` stays exactly right; **dropping the `sm_scale` fold** (forgetting to multiply `dS` by `sm_scale`, or applying it twice) makes `dQ` and `dK` off by a factor of $\sqrt d$ while `dV` — which is computed from `P` and `dO` alone and never touches `ds` — stays correct. That asymmetry is the tell in both cases.
